...
BSP_DDR_SECT_INFO g_stDdrSectInfo[BSP_DDR_SECT_TYPE_BUTTOM] = {
    ...
    {BSP_DDR_SECT_TYPE_WAN,       BSP_DDR_SECT_ATTR_NONCACHEABLE, (void*)0,                 (void*)0,               SHM_MEM_WAN_SIZE},
    {BSP_DDR_SECT_TYPE_SHARE_MEM, BSP_DDR_SECT_ATTR_NONCACHEABLE, (void*)0,         (void*)DDR_SHARED_MEM_ADDR,            DDR_SHARED_MEM_SIZE},
    {BSP_DDR_SECT_TYPE_EXCP,      BSP_DDR_SECT_ATTR_NONCACHEABLE, (void*)HI_IO_ADDRESS(DDR_MNTN_ADDR),     (void*)DDR_MNTN_ADDR,                  DDR_MNTN_SIZE},
...
{
    g_stAxiSectInfo[BSP_AXI_SECT_TYPE_TEMPERATURE].pSectVirtAddr = SRAM_TEMP_PROTECT_ADDR;
   // g_stAxiSectInfo[BSP_AXI_SECT_TYPE_TEMPERATURE].pSectPhysAddr = SRAM_TEMP_PROTECT_ADDR - HI_SRAM_MEM_ADDR_VIRT + HI_SRAM_MEM_ADDR;
    g_stAxiSectInfo[BSP_AXI_SECT_TYPE_TEMPERATURE].pSectPhysAddr = (void*)SRAM_V2P(SRAM_TEMP_PROTECT_ADDR);
    g_stAxiSectInfo[BSP_AXI_SECT_TYPE_DICC].pSectVirtAddr = MEMORY_AXI_DICC_ADDR;
   //g_stAxiSectInfo[BSP_AXI_SECT_TYPE_DICC].pSectPhysAddr = (u32)MEMORY_AXI_DICC_ADDR  - HI_SRAM_MEM_ADDR_VIRT + HI_SRAM_MEM_ADDR;
    g_stAxiSectInfo[BSP_AXI_SECT_TYPE_DICC].pSectPhysAddr = (void*)SRAM_V2P(MEMORY_AXI_DICC_ADDR);
    g_stAxiSectInfo[BSP_AXI_SECT_TYPE_HIFI].pSectVirtAddr = SHM_MEM_HIFI_ADDR;
    g_stAxiSectInfo[BSP_AXI_SECT_TYPE_HIFI].pSectPhysAddr = (void*)SHD_DDR_V2P(SHM_MEM_HIFI_ADDR);
    g_stAxiSectInfo[BSP_AXI_SECT_TYPE_TTF_BBP].pSectVirtAddr = SRAM_GU_MAC_HEADER_ADDR;
    g_stAxiSectInfo[BSP_AXI_SECT_TYPE_TTF_BBP].pSectPhysAddr = (void*)SRAM_V2P(SRAM_GU_MAC_HEADER_ADDR);

    g_stDdrSectInfo[BSP_DDR_SECT_TYPE_TTF].pSectVirtAddr = ECS_TTF_BASE_ADDR_VIRT;
    g_stDdrSectInfo[BSP_DDR_SECT_TYPE_NV].pSectVirtAddr = ECS_NV_BASE_ADDR_VIRT;
    g_stDdrSectInfo[BSP_DDR_SECT_TYPE_DICC].pSectVirtAddr = ECS_TTF_DICC_ADDR_VIRT;
    g_stDdrSectInfo[BSP_DDR_SECT_TYPE_WAN].pSectVirtAddr = SHM_MEM_WAN_ADDR;
    g_stDdrSectInfo[BSP_DDR_SECT_TYPE_WAN].pSectPhysAddr = (void*)SHD_DDR_V2P(SHM_MEM_WAN_ADDR);
    g_stDdrSectInfo[BSP_DDR_SECT_TYPE_SHARE_MEM].pSectVirtAddr = DDR_SHARED_MEM_VIRT_ADDR;
...
BSP_VOID BSP_DDR_ShowSectInfo(BSP_VOID)
{
    ...
    {
        printk("0x%-8.8x 0x%p 0x%p 0x%-8.8x 0x%-8.8x\n\n", \
...
void* DRV_DDR_VIRT_TO_PHY(void* ulVAddr)
{
    ...
    if((ulVAddr >= DDR_GU_ADDR_VIRT)
        && (ulVAddr <= (void*)((unsigned long)DDR_GU_ADDR_VIRT + DDR_GU_SIZE)))
    {
        return (void*)((unsigned long)ulVAddr - (unsigned long)DDR_GU_ADDR_VIRT + (unsigned long)DDR_GU_ADDR);
    }
...
{
    if((ulPAddr >= (void*)((unsigned long)DDR_SHARED_MEM_ADDR))
        && (ulPAddr < (void*)((unsigned long)DDR_SHARED_MEM_ADDR + DDR_SHARED_MEM_SIZE)))
    {
        return (void*)SHD_DDR_P2V(ulPAddr);
    }

    if((ulPAddr >= (void*)((unsigned long)DDR_GU_ADDR))
        && (ulPAddr < (void*)((unsigned long)DDR_GU_ADDR + DDR_GU_SIZE)))
    {
        return (void*)(ulPAddr - (void*)((unsigned long)DDR_GU_ADDR) + DDR_GU_ADDR_VIRT);
    }
...
{
    return (ulVAddr - ECS_TTF_BASE_ADDR_VIRT + (void*)ECS_TTF_BASE_ADDR);
}
...
{
    return (ulPAddr - (void*)ECS_TTF_BASE_ADDR + (void*)ECS_TTF_BASE_ADDR_VIRT);
}
...
{
    return (ulVAddr - HI_SRAM_MEM_ADDR_VIRT + (void*)HI_SRAM_MEM_ADDR);
}
...
{
    return (ulPAddr - (void*)HI_SRAM_MEM_ADDR + HI_SRAM_MEM_ADDR_VIRT);
}
...
void show_shared_ddr_status(void)
{
	/*请依照先后顺序增加打印输出项*/
        unsigned long total_size = SHM_MEM_APPA9_PM_BOOT_SIZE + SHM_MEM_MDMA9_PM_BOOT_SIZE + SHM_MEM_SYNC_SIZE          + SHM_MEM_ICC_SIZE +
                     ...
                     SHM_MEM_MODEM_PINTRL_SIZE  + SHM_MEM_TEMPERATURE_SIZE   +SHM_MEM_RESTORE_AXI_SIZE +
	...
	printf("%-30s%10x%10p%10x\n", "CORESHARE_MEM_TENCILICA_MULT_BAND_ADDR", CORESHARE_MEM_TENCILICA_MULT_BAND_ADDR, (void*)SHD_DDR_P2V(CORESHARE_MEM_TENCILICA_MULT_BAND_ADDR), CORESHARE_MEM_TENCILICA_MULT_BAND_SIZE);/*lint !e778 */
