...
bool nv_mtcarrier_test_flag = FALSE;
void nv_mtcarrier_backup_test1(void)
{
    u32 ret = NV_ERROR;
    char *path = NULL;

    nv_mtcarrier_test_flag = TRUE;

    ret = bsp_nvm_mtcarrier_backup(path);

    if(NV_OK != ret)
    {
        printk("%s:test fail, ret is %d\n",__FUNCTION__,ret);
    }

    nv_mtcarrier_test_flag = FALSE;
    
}

void nv_mtcarrier_custxml_test1(void)
{
    u32 ret = NV_ERROR;
...
void nv_mtcarrier_revert_test1(void)
{
    u32 ret = NV_ERROR;
