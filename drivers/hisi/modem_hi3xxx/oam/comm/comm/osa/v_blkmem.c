...
/*                                                                           */
/*****************************************************************************/
/*lint --e{537,958,740,713,732,718,746,830,539}*/
...
/*for v7r2 dump begin*/
VOS_VOID DUMP_MemBlock(VOS_VOID);
/*for v7r2 dump end*/
...
/*for v7r2 dump begin*/
    ...
    DRV_DUMP_SAVE_REGISTER_HOOK(DUMP_SAVE_MOD_OSA_MEM, DUMP_MemBlock);
                ...
                VOS_INT32 ulLineNo,VOS_MEM_CTRL_BLOCK *pstMemCtrl, VOS_INT lCycle)
{
    ...
    VOS_MemSet(pulDumpBuffer, 0, VOS_DUMP_MEM_TOTAL_SIZE); /* [false alarm]: 屏蔽Fortify错误 */

    *pulDumpBuffer++ = ulFileID; /* [false alarm]: 屏蔽Fortify错误 */
    *pulDumpBuffer++ = (VOS_UINT32)ulLineNo; /* [false alarm]: 屏蔽Fortify错误 */
    *pulDumpBuffer++ = ulInfo; /* [false alarm]: 屏蔽Fortify错误 */
    *pulDumpBuffer++ = ulSize; /* [false alarm]: 屏蔽Fortify错误 */
    *pulDumpBuffer++ = VOS_GetSlice(); /* [false alarm]: 屏蔽Fortify错误 */
    ...
    for ( i=lCycle-1; i>=0; i--)
    {
        ...
        {
            *pulDumpBuffer++ = pstTmpMemHead->aulMemRecord[0]; /* [false alarm]: 屏蔽Fortify错误 */
            *pulDumpBuffer++ = pstTmpMemHead->aulMemRecord[1]; /* [false alarm]: 屏蔽Fortify错误 */
            *pulDumpBuffer++ = pstTmpMemHead->ulAllocPid; /* [false alarm]: 屏蔽Fortify错误 */
            *pulDumpBuffer++ = pstTmpMemHead->ulAllocSize; /* [false alarm]: 屏蔽Fortify错误 */
            *pulDumpBuffer++ = pstTmpMemHead->ulcputickAlloc; /* [false alarm]: 屏蔽Fortify错误 */
...
VOS_VOID DUMP_MemBlock(VOS_VOID)
{
    VOS_INT i;
    VOS_INT offset = 0;
    VOS_UINT32 *pstBuf = VOS_NULL;
    VOS_UINT32 stBufLen = 0;
    VOS_MEM_CTRL_DUMP_BLOCK  *pBlocks = VOS_NULL;
    VOS_MEM_HEAD_BLOCK  *pstTemp = VOS_NULL;
    VOS_MEM_GLOBAL_DUMP_INFO GlobalDumpInfo ;

    /* count msg mem dump info size */
    GlobalDumpInfo.MsgMemOffset = sizeof(VOS_MEM_GLOBAL_DUMP_INFO);
    GlobalDumpInfo.MsgMemSize  = VOS_MEM_CTRL_BLOCK_NUMBER *sizeof(VOS_MEM_CTRL_DUMP_BLOCK);
    GlobalDumpInfo.MsgMemBlockNum = VOS_MEM_CTRL_BLOCK_NUMBER;

    /* count simple mem dump info size */
    GlobalDumpInfo.SimpleMemOffset = GlobalDumpInfo.MsgMemOffset + GlobalDumpInfo.MsgMemSize;
    GlobalDumpInfo.SimpleMemSize =VOS_SIMPLE_MEM_CTRL_BLOCK_NUMBER *sizeof(VOS_MEM_CTRL_DUMP_BLOCK);
    GlobalDumpInfo.SimpleMemBlockNum = VOS_SIMPLE_MEM_CTRL_BLOCK_NUMBER;

   if(DRV_DUMP_GET_BUFFER(DUMP_SAVE_MOD_OSA_MEM,(VOS_CHAR**)&pstBuf,(BSP_U32*)&stBufLen))
    {
        return ;
    }

    if(stBufLen < (GlobalDumpInfo.MsgMemSize + GlobalDumpInfo.SimpleMemSize + sizeof(VOS_MEM_GLOBAL_DUMP_INFO)))
    {
        return;
    }

    VOS_MemSet(pstBuf, 0, stBufLen);

    VOS_MemCpy(pstBuf, &GlobalDumpInfo,  sizeof(VOS_MEM_GLOBAL_DUMP_INFO));

    offset += sizeof(VOS_MEM_GLOBAL_DUMP_INFO);

     /*select msg mem info*/
    for(i = 0;i < VOS_MEM_CTRL_BLOCK_NUMBER;i++)
    {
        pBlocks = (VOS_MEM_CTRL_DUMP_BLOCK *)((VOS_UINT8 *)pstBuf + offset);

        pBlocks->BlockLength = VOS_MemCtrlBlk[i].BlockLength ;
        pBlocks->TotalBlockNumber = VOS_MemCtrlBlk[i].TotalBlockNumber;
        pBlocks->IdleBlockNumber   = VOS_MemCtrlBlk[i].IdleBlockNumber;
        pBlocks->BuysBlockNumber  = VOS_MemCtrlBlk[i].TotalBlockNumber - VOS_MemCtrlBlk[i].IdleBlockNumber;
        pBlocks->PidNumber             = VOS_PID_BUTT-VOS_PID_DOPRAEND;

        pstTemp = VOS_MemCtrlBlk[i].BusyBlocks;
        while ( VOS_NULL_PTR != pstTemp)
        {

            if((pstTemp->ulAllocPid >=VOS_PID_DOPRAEND )&&(pstTemp->ulAllocPid <= VOS_PID_BUTT ))
            {
                pBlocks->PidInfo[pstTemp->ulAllocPid -VOS_PID_DOPRAEND].AllocPid = (VOS_UINT16)pstTemp->ulAllocPid;
                pBlocks->PidInfo[pstTemp->ulAllocPid -VOS_PID_DOPRAEND].AllocNum++;
            }
            else
            {
                //vos_printf(" msg pstTemp->ulAllocPid = %d,VOS_PID_DOPRAEND = %d \n",pstTemp->ulAllocPid,VOS_PID_DOPRAEND);
            }
        ...
        offset += sizeof(VOS_MEM_CTRL_DUMP_BLOCK) ;

    }

    /* select simple mem dump info*/

    for(i = 0;i < VOS_SIMPLE_MEM_CTRL_BLOCK_NUMBER;i++)
    {
        pBlocks = (VOS_MEM_CTRL_DUMP_BLOCK *)((VOS_UINT8 *)pstBuf + offset);

        pBlocks->BlockLength = VOS_SimpleMemCtrlBlk[i].BlockLength ;
        pBlocks->TotalBlockNumber = VOS_SimpleMemCtrlBlk[i].TotalBlockNumber;
        pBlocks->IdleBlockNumber   = VOS_SimpleMemCtrlBlk[i].IdleBlockNumber;
        pBlocks->BuysBlockNumber  = VOS_SimpleMemCtrlBlk[i].TotalBlockNumber - VOS_SimpleMemCtrlBlk[i].IdleBlockNumber;
        ...
        while ( VOS_NULL_PTR != pstTemp)
        {
           if((pstTemp->ulAllocPid >=VOS_PID_DOPRAEND )&&(pstTemp->ulAllocPid <= VOS_PID_BUTT ))
            ...
            {
                //vos_printf(" simple pstTemp->ulAllocPid = %d,VOS_PID_DOPRAEND = %d \n",pstTemp->ulAllocPid,VOS_PID_DOPRAEND);
            }
            pstTemp = pstTemp->pstNext;
