...
VOS_UINT32 Diag_AirMessageTest( VOS_UINT32 ulSize)
{
	...
	if(g_HsoConn)
	{
		diag_CfgResetAllSwt();/* [false alarm]:屏蔽Fortify */
...
VOS_UINT32 Diag_GuAirMessageTest( VOS_UINT32 ulSize)
{
	...
	pRptMessage = (VOS_UINT8*) diag_HeapAllocD(sizeof(DIAG_GU_AIR_MSG_LOG_STRU)+100);   /* [false alarm]:屏蔽Fortify */
    /*lint -save -e830 -e539*/
    if(NULL == pRptMessage)
    {
        return ERR_MSP_FAILURE;
	...
	ret = DIAG_ReportGUAirMessageLog(pRptMessage,ulMsgLen);/* [false alarm]:屏蔽Fortify */
	return ret;/* [false alarm]:屏蔽Fortify */
}
...
VOS_UINT32 Diag_PrintMessageTest( VOS_UINT32 ulLevelFilter)
{
	...
	if(g_HsoConn)
	{
		...
		ret = diag_CfgSetGlobalBitValue(&g_ulDiagCfgInfo,DIAG_CFG_CONN_BIT,DIAG_CFG_SWT_OPEN);/* [false alarm]:屏蔽Fortify */
		if(ret != ERR_MSP_SUCCESS)
		{
...
VOS_UINT32 Diag_CmdMessageTest( VOS_UINT32 id,VOS_UINT32 ulSize)
{
    ...
    /*lint -save -e360 -e734*/
	ret = DIAG_ReportCommand(id,ulSize,ucMsg);
	return ret;
    /*lint -restore*/
}
