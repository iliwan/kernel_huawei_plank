...
/*                                                                           */
/*****************************************************************************/
/*lint --e{958,537,613,740,732,701,774}*/
#include "v_typdef.h"
#include "v_timer.h"
#include "v_msg.h"
#include "v_task.h"
        ...
        if ( head_Ptr != VOS_NULL_PTR)
        {
                ...
                && ( 0 == head_Ptr->TimeOutValueInTicks ) )
            {
                ...
                *(head_Ptr->phTm) = VOS_NULL_PTR;
                ...
                {
                    vos_Timer_expire_tail_Ptr->next = vos_TimerCtrlBlkCurrent; /* [false alarm]: 屏蔽Fortify错误 */
                    vos_Timer_expire_tail_Ptr = vos_TimerCtrlBlkCurrent;
                }
...
/*****************************************************************************
 Function   : VOS_TmNow
 Description: Get the number of milliseconds that have elapsed since the
              system was started
 Input      : None
 Output     : pulMillSecHigh -- The high 32 bits of the number of
                                milliseconds that have elapsed
                                since the system was started
              pulMillSecLow  -- The low 32 bits of the number of
                                milliseconds that have elapsed
                                since the system was started
 Return     : VOS_OK on success, error code on failure
 *****************************************************************************/
...
{
    *pulRetTimeInMillSecHigh = vos_CPU_TICK.ulHigh * MILLISECONDS_PER_TICK;
    *pulRetTimeInMillSecLow = vos_CPU_TICK.ulLow * MILLISECONDS_PER_TICK;
    return VOS_OK;
}
#endif /* DMT */
