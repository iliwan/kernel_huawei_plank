...
static int get_image_version(char *buf)
{
    ...
    if(!drv_data) {
        ...
        hwlog_err("%s, %d : drv_data is NULL!\n", __func__, __LINE__);
    ...
    for(i = 0; i < RETRY_TIME; i++) {
        ...
        if(ret) {
            hwlog_err("%s, %d : read version fail, ret = %d !\n", __func__, __LINE__, ret);
            return SPI_OUTPUT_DATA_LEN;
        }
        ret = snprintf(offset, SPI_OUTPUT_DATA_LEN, "%02x", read_version[3]);
        if(ret < 0) {
            hwlog_err("%s, %d : snprintf err, ret = %d!\n", __func__, __LINE__, ret);
            return SPI_OUTPUT_DATA_LEN;
        }
        offset += ret;
        ...
        if(!strncasecmp(buf,INVALID_VERSION1 , SPI_OUTPUT_DATA_LEN) || !strncasecmp(buf,INVALID_VERSION2 , SPI_OUTPUT_DATA_LEN) ) {
            hwlog_err("%s, %d : ret = %d, get version %s \n", __func__, __LINE__, ret, buf);
            msleep(10);
            continue;
        }else{
            hwlog_info("%s, %d : FPGA:read version register successfully!\n", __func__, __LINE__);
            break;
        }
    }
    return SPI_OUTPUT_DATA_LEN;
...
int check_input_exc(const char *inputdata, const char *outputdata)
{
    ...
    for(i = 0; i < RETRY_TIME; i++){
        ...
        if(ret) {
            hwlog_err("%s, %d : read testmode register fail, ret = %d!\n", __func__, __LINE__, ret);
        }
        ...
        offset += ret;
        ret = snprintf(offset, SPI_OUTPUT_DATA_LEN, "%02x", outdata[6]);

        if(simple_strtoul(buf ,NULL, BASE_HEX) == simple_strtoul(outputdata, NULL, BASE_HEX)) {
            hwlog_info("%s, %d : Output is correct: inputdata = %s; expect outputdata = %s;return data = %s\n", __func__, __LINE__, inputdata, outputdata, buf);
            return SUCCESS;
        } else {
            hwlog_err("%s, %d : ret = %d, Output is error: inputdata = %s;expect outputdata = %s;return data = %s\n", __func__, __LINE__, ret,  inputdata, outputdata, buf);
            msleep(10);
            continue;
        }
    }
