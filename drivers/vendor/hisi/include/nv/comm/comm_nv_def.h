...
/*****************************************************************************
 ...
 结构说明  : en_NV_Item_FEM_GPIO_MIPI_CTRL 结构 ID= 18002
*****************************************************************************/
...
{	
    ...
    } pin_mask;              /*射频管脚掩码,0表示该bit位不需要控制*/
    union
    {
        BSP_U32              level;
        FEMIO_PIN_CFG_STRU   cfg;
    } pin_level;              /*射频管脚输出电平 值,0表示该低电平，1表示高电平*/
...
typedef struct
{  
	BSP_U32             count;                    /*有效的mipi 指令个数*/
	FEM_MIPI_CFG_STRU   cmd[FEM_MIPI_CMD_MAX];	  /*mipi控制字*/
...
typedef struct
{
	...
	FEMIO_GPIO_CTRL_STRU  gpio_ctrl;/*GPIO 配置  */
