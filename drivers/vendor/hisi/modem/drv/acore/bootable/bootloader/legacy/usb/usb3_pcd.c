...
intf = {
	...
	USB_SC_FASTBOOT, /* bInterfaceSubClass */
	USB_PR_FASTBOOT, /* bInterfaceProtocol */
	0, /* iInterface */
};
...
uint32_t usb3_get_device_speed(usb3_pcd_t *pcd)
{
	...
	dsts = dwc_readl(&pcd->dev_global_regs->dsts);
...
static void usb3_do_set_config(usb3_pcd_t *pcd)
{
    ...
    if (!wvalue ||(wvalue == CONFIG_VALUE)) {
        ...
        if (wvalue) {            
            ...
            /* Prepare to receive fastboot cmd */
            usb_status(wvalue, pcd->speed);
...
static void usb3_do_get_descriptor(usb3_pcd_t *pcd)
{
    ...
    case UDESC_DEVICE:
        {
            ...
            dev->idProduct = FASTBOOT_PRODUCT_ID;
            dev->bcdDevice = 0x0001;
    ...
    case UDESC_CONFIG:
        {
            ...
            buf += sizeof(*config);
            memcpy((void *)buf, (void *)&intf, sizeof(intf));
            buf += sizeof(intf);
    ...
    case UDESC_STRING:
        {
            ...
            case STRING_MANUFACTURER:
                ...
                memcpy((void *)(buf+2), (void *)usb3_dev->string_manu,
            ...
            case STRING_PRODUCT:
                ...
                memcpy((void *)(buf+2), (void *)usb3_dev->string_prod,
	...
	case UDESC_BOS:
		...
		memcpy((void *)buf, (void *)&bos, sizeof(bos));
		buf += sizeof(bos);
		memcpy((void *)buf, (void *)&cap1, sizeof(cap1));
		buf += sizeof(cap1);
		memcpy((void *)buf, (void *)&cap2, sizeof(cap2));
		buf += sizeof(cap2);
		memcpy((void *)buf, (void *)&cap3, sizeof(cap3));
			 ...
			 usb3_pcd_req_t *req)
{
	...
	if (ep == &pcd->ep0) {
		if (ep->is_in) {
			...
			req->trbdma = (uint32_t)pcd->ep0_in_desc;
		} else {
			req->trb = pcd->ep0_out_desc;
			req->trbdma = (uint32_t)pcd->ep0_out_desc;
		}
	/* Else fill request with TRB from the non-EP0 allocation */
	} else {
		req->trb = ep->ep_desc;
		req->trbdma = (uint32_t)ep->ep_desc;
	}
}
...
void usb3_ep0_start_transfer(usb3_pcd_t *pcd, usb3_pcd_req_t *req)
{
	...
	if (ep0->is_in) {
		...
		usb3_fill_desc(desc, (uint32_t)req->bufdma,
				   len, 0, desc_type,
...
static void ep0_continue_transfer(usb3_pcd_t *pcd, usb3_pcd_req_t *req)
{
	...
	if (ep0->is_in) {
		...
		desc_dma = (uint32_t)pcd->ep0_in_desc;
		ep_reg = ep0->in_ep_reg;
...
{
    struct usb_request *request = ep->ept->req;
    
	if (ep != &pcd->ep0)
		req->trb = NULL;

    /* Execute callback from protocol layer */
	if (request->complete)
        request->complete(request, req->actual, status);
                 ...
                 usb3_pcd_ep_t *ep, uint32_t event)
{
    ...
    /* 
     *  IMPORTANT NOTE:
     *  1. Based on transfer level;
     *  2. Reponse length is less than 64 bytes;
     *  3. Appreciated length is no more than 4096 bytes;
     */
