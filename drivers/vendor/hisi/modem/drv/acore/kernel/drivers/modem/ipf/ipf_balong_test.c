...
BSP_S32 IpfDlIntCb301(BSP_VOID)
{
	...
	{	
		...
		s32Ret = CmpDMAData((BSP_U32 *)g_pu8STDlData, (BSP_U32 *)phys_to_virt((unsigned long)stRd[j].u32OutPtr), stRd[j].u16PktLen);
...
#ifdef CONFIG_ARM64
		ADdlshort[i].u32OutPtr1 = virt_to_phys(p_vir_addr);
#else
		ADdlshort[i].u32OutPtr1 = (BSP_U32)(unsigned long)p_vir_addr;
#endif
		

		dma_map_single(&(balong_device_ipf.dev), p_vir_addr, 438, DMA_FROM_DEVICE);
...
#ifdef CONFIG_ARM64
		ADdllong[i].u32OutPtr1 = virt_to_phys(p_vir_addr);
#else
		ADdllong[i].u32OutPtr1 = (BSP_U32)(unsigned long)p_vir_addr; 
#endif

		dma_map_single(&(balong_device_ipf.dev), p_vir_addr ,1500,DMA_FROM_DEVICE);
