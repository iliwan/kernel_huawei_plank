...
int pm_msg_parse_ccore(unsigned int typeid, char *in_buf, unsigned int len, char *out_buf, unsigned int *out_ptr)
{
	...
	{	
		...
		data_len = snprintf(&out_buf[*out_ptr], PMOM_PARSE_STR_BUFSIZ, 
				"suspend_start:0x%x, suspend_end:0x%x, suspend_max:0x%x(s_max_start = 0x%x, s_max_end=0x%x), resume_start:0x%x, resume_end:0x%x, resume_max=0x%x,dpm_suspend_duration=0x%x,dpm_resume_duration = 0x%x,dpm_fail_count = 0x%x\n", 
				...
				log_msg->resume_start_time, log_msg->resume_end_time,log_msg->resume_max_time,log_msg->dpm_suspend_duration,log_msg->dpm_resume_duration,log_msg->dpm_fail_count); 
		*out_ptr = *out_ptr + data_len;
	}
