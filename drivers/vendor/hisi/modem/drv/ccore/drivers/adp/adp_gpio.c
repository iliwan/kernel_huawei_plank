...
unsigned int drv_gpio_oprt(unsigned long ulOp, unsigned char *pucPL)
{
    unsigned int i = 0;
    unsigned int j = 0;
...
int bsp_pinctrl_mux_set(unsigned int gpio,unsigned int type)
{
    unsigned int base_addr = 0;
    struct rf_gpio_cfg *cfg = get_global_rf_gpio_cfg(gpio);
    ...
    writel(type, base_addr + cfg->mux_addr);
...
void bsp_dpm_powerup_antn_config(void)
{
    ...
    /*打开天线开关*/
    (void)bsp_pmu_fem_poweron(PWC_COMM_MODEM_0);
    (void)bsp_pmu_fem_poweron(PWC_COMM_MODEM_1);
...
int mdrv_dpdt_gpio_set_value(unsigned int gpio,unsigned int value)
{
...
int mdrv_dpdt_gpio_get_value(unsigned int gpio)
{
	return bsp_gpio_get_value(gpio);
}


int mdrv_dpdt_pinctrl_config(unsigned int gpio,unsigned int type)
{
    unsigned int base_addr = 0;
    struct rf_gpio_cfg *cfg = get_global_rf_gpio_cfg(gpio);
...
{   
    ...
    if(OUTER_RFSWITCH_ON == status)
    {
        for( i = 0 ; i < RF_GPIO_NV_MAX ; i++ )
        {
            ...
            {
                bsp_pinctrl_mux_set(gpio_outer[i].gpio, 0);
...
int outer_rfswitch_get(unsigned int *status)
{
    ...
    for( i = 0 ; i < RF_GPIO_NV_MAX ; i++ )
    {
        ...
        if(0 == is_in_global_cfg_table(gpio_outer[i].gpio) && (0 == bsp_pintrl_mux_get_without_dts(gpio_outer[i].gpio, &func)))
        {
            func_t |= func;
