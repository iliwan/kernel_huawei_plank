...
static s32 hardtimer_start_ms(struct bsp_hardtimer_control  *timer_ctrl)
{
	...
	ret = bsp_hardtimer_enable(timer_id);/* [false alarm]:误报 */
	return ret;
}
...
{
	u32 i=0,min = 0xffffffff,ret = 0;/*lint !e123*/
...
#if defined(CONFIG_CCORE_PM)&&defined(K3_TIMER_FEATURE)
#define MODEM_TIMER_NUM 8
struct timer_dpm_s
{
	u32 u32_ctrl_regbak[MODEM_TIMER_NUM];
	u32 u32_initvalue_regbak[MODEM_TIMER_NUM];
};
static struct timer_dpm_s timer_dpm;
s32 hardtimer_suspend_late(struct dpm_device *dev)
{
	u32 i = 0;
	for(i=0;i<MODEM_TIMER_NUM;i++)
	{
		timer_dpm.u32_initvalue_regbak[i]  = readl(hard_timer_control[i].addr+HI_TIMER_LOADCOUNT_OFFSET);
		timer_dpm.u32_ctrl_regbak[i] = readl(hard_timer_control[i].addr+HI_TIMER_CONTROLREG_OFFSET);		
	}
	return OK;
}

s32 hardtimer_resume_early(struct dpm_device *dev)
{
	u32 i = 0;
	unsigned int regvalue;
	/*modem timer set clk,0/3/5/6/7 32KHZ*/
	/*disable clk*/
	writel(0x74800000,HI_SYSCTRL_BASE_ADDR+0x28);
	regvalue = readl(HI_SYSCTRL_BASE_ADDR+0x144);
	writel(regvalue&(~0xe9),HI_SYSCTRL_BASE_ADDR+0x144);
	/*enable clk*/
	writel(0x74800000,HI_SYSCTRL_BASE_ADDR+0x24);
	for(i=0;i<MODEM_TIMER_NUM;i++)
	{
		(void)bsp_hardtimer_disable(i);
		writel(timer_dpm.u32_initvalue_regbak[i],hard_timer_control[i].addr+HI_TIMER_LOADCOUNT_OFFSET);
		writel(timer_dpm.u32_ctrl_regbak[i],hard_timer_control[i].addr+HI_TIMER_CONTROLREG_OFFSET);
	}
	return OK;
	
}
struct dpm_device k3_timer_device =
{
	.device_name = "k3_timer_device",
	.suspend_late = hardtimer_suspend_late,
	.resume_early = hardtimer_resume_early,
};
#endif
...
void timer_dpm_init(void)
{
	...
	#if defined(CONFIG_CCORE_PM)&&defined(K3_TIMER_FEATURE)
	if(bsp_device_pm_add(&k3_timer_device))
	{
		hardtimer_print_error("timer_dpm_init failed, dpm add error\n");
		return;
	}
	else
	{
		hardtimer_print_error("timer_dpm_init ok\n");
	}
	#else
	return;
	#endif
