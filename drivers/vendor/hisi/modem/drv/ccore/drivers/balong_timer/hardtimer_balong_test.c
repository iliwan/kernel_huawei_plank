...
s32 hardtimer_test_case16(void);
s32 hardtimer_test_case17(void);
s32 hardtimer_test(void);

s32 adp_test_case01(void);
s32 adp_test_case02(void);
s32 adp_test_case03(void);
s32 adp_test_case04(void);
s32 adp_test_case05(void);
s32 adp_test_case06(void);
s32 adp_test_case07(void);
...
s32 hardtimer_test_case01(void)
{
   ...
   my_timer.timerId = i;
   my_timer.para = i;
   osl_sem_init(SEM_EMPTY,&sem);/*lint !e129*//*lint !e40*/
   ...
   if (OK != ret)
   	return ERROR;
  osl_sem_down(&sem);/*lint !e129*//*lint !e40*/
...
s32 adp_test_case01(void)
{
	s32 ret = 0;
	osl_sem_init(SEM_EMPTY,&sem);/*lint !e40*/
	ret = DRV_TIMER_START(TIMER_CCPU_DSP_DRX_PROT_ID,func,0,1000,0,TIMER_UNIT_MS);
	if(ret!=0)
	{
	...
	osl_sem_down(&sem);/*lint !e40*/
	ret = DRV_TIMER_STOP(TIMER_CCPU_DSP_DRX_PROT_ID);
	if(ret!=0)
	{
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"DRV_TIMER_START fail\n");
		return ERROR;
...
s32 adp_test_case02(void)
{
	...
	osl_sem_init(SEM_EMPTY,&sem);/*lint !e40*/
	ret = DRV_TIMER_START(TIMER_DSP_TIMER2_ID,func,0,10000,0,TIMER_UNIT_NONE);
	if(ret!=0)
	{
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"DRV_TIMER_START fail\n");
		return ERROR;
	}
	ret = DRV_TIMER_GET_REST_TIME(TIMER_DSP_TIMER2_ID,TIMER_UNIT_NONE,&restTime);
	if(ret!=0)
	{
...
s32 adp_test_case04(void)
{
	...
	osl_sem_down(&sem);/*lint !e40*/
	ret = DRV_TIMER_STOP(TIMER_CCPU_OSA_ID);
	if(ret!=0)
	{
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"DRV_TIMER_START fail\n");
		return ERROR;
...
s32 hardtimer_test(void)
{
   ...
   ret = hardtimer_test_case16();
   if (OK == ret)
   	bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"hardtimer_test_case16 pass\n");
   else
   	bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"hardtimer_test_case16 fail\n");
   ret = hardtimer_test_case17();
   if (OK == ret)
   	bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"hardtimer_test_case17 pass\n");
   else
   	bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"hardtimer_test_case17 fail\n");
...
void adp_test(void)
{
	...
	ret = adp_test_case03();
	if (OK == ret)
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"adp_test_case03 pass\n");
	else
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"adp_test_case03 fail\n");
	ret = adp_test_case04();
	if (OK == ret)
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"adp_test_case04 pass\n");
	else
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"adp_test_case04 fail\n");
	ret = adp_test_case05();
	if (OK == ret)
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"adp_test_case05 pass\n");
	else
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"adp_test_case05 fail\n");
	ret = adp_test_case06();
	if (OK == ret)
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"adp_test_case06 pass\n");
	 else
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"adp_test_case06 fail\n");
	ret = adp_test_case07();
	if (OK == ret)
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"adp_test_case07 pass\n");
	else
		bsp_trace(BSP_LOG_LEVEL_ERROR,BSP_MODU_HARDTIMER,"adp_test_case07 fail\n");
	return ;
}
