...
#define DPM_DUMP_START      (dpm_dump_drx_c_addr+0x300)
#define DPM_DUMP_SIZE       (0x300)
...
void dpm_stamp_fail_cnt(u32 fail_cnt)
{
    ...
    {
        writel( fail_cnt, DPM_STAMP_START+g_dpm_index*0x10+0xC);
        bsp_pm_log(PM_OM_DPM, 0x10, (void*)(DPM_STAMP_START+g_dpm_index*0x10));
    }
}
...
/*lint --e{10,26,64,413,718,746}*//*for:list_for_each_entry_safe*/
...
{
	unsigned long flags = 0;
	struct dpm_device *dev=NULL,*next=NULL;
	s32 ret=0;
	spin_lock_irqsave(&(dpm_ctrl.dpm_list_mtx),flags);/*lint !e40*/
...
void bsp_dpm_show()
{
	...
	{
		dpm_print("\t\t%s:\n",dev->device_name);
		dpm_print("\t prepare   time begin:0x%x end:0x%x\n",dev->debug_time[PREPARE][0]  ,dev->debug_time[PREPARE][1]);
		dpm_print("\t sus early time begin:0x%x end:0x%x\n",dev->debug_time[SUS_EARLY][0],dev->debug_time[SUS_EARLY][1]);
		dpm_print("\t suspend   time begin:0x%x end:0x%x\n",dev->debug_time[SUSPEND][0]  ,dev->debug_time[SUSPEND][1]);
		dpm_print("\t sus late  time begin:0x%x end:0x%x\n",dev->debug_time[LATE][0]     ,dev->debug_time[LATE][1]);
		dpm_print("\t res early time begin:0x%x end:0x%x\n",dev->debug_time[EARLY][0]    ,dev->debug_time[EARLY][1]);
		dpm_print("\t resume    time begin:0x%x end:0x%x\n",dev->debug_time[RESUME][0]   ,dev->debug_time[RESUME][1]);
		dpm_print("\t res  late time begin:0x%x end:0x%x\n",dev->debug_time[RES_LATE][0] ,dev->debug_time[RES_LATE][1]);
		dpm_print("\t complete  time begin:0x%x end:0x%x\n",dev->debug_time[COMPLETE][0] ,dev->debug_time[COMPLETE][1]);
...
void bsp_dpm_init(void)
{
    ...
    dpm_dump_drx_c_addr = (u32)DUMP_EXT_OM_DRX_CCORE_ADDR;
    if(0!=dpm_dump_drx_c_addr)
    {
        memset((void*)(DPM_DUMP_START),0,DPM_DUMP_SIZE);
    }
    else
    {
