...
/*lint --e{438,525,537,539,701,713,732,737,746,830,958}*/
...
//extern u32 bsp_mod_level_set(bsp_module_e mod_id, u32 print_level);//bsp_om.h包含
...
s32 bsp_edma_channel_2vec_start (u32 channel_id, u32 src_addr, u32 des_addr, u32 len, u32 size_align)
{
    ...
    else
    {
        ...
        lli_node_tmp->des_addr  = des_addr;
        edma_cache_sync();
        lli_node_tmp->config   = (reg_value | EDMA_CHANNEL_ENABLE);/* [false alarm]:误报 */
        edma_cache_sync();
        return EDMA_SUCCESS;
    }
}
...
/*
专为gu 临时版本: edma16 - channel 2 6 7
                 edma 4 - channel 0 1 2 3
*/
/*
u32 edma_chans_suspend[] = { EDMA_CH_GUDSP_MEMORY_3,
                                EDMA_CH_GUDSP_MEMORY_9,
                                EDMA_CH_GUDSP_MEMORY_7,
                                EDMA_CH_GUDSP_MEMORY_1,
                                EDMA_CH_GUDSP_MEMORY_2,
                                EDMA_CH_GUDSP_MEMORY_4,
                                EDMA_CH_GUDSP_MEMORY_6,
                                };
u32 channels_suspend_mask
*/
u32 edma_chans_suspend_mask[EDMA_NUMBER];
struct EDMAC_TRANSFER_CONFIG_STRU edma_chan_cfg_bak[7];/* [sizeof(edma_chans_suspend)/sizeof(u32)]; */
EDMAC_CPU_REG_STRU edma_cpu_reg_bak[EDMA_NUMBER];


u32 edma_16_chan_saved[3] = {2,6,7};
u32 edma_16_chan_mask = 0xc4;
#ifdef HI_EDMA_CH4_REGBASE_ADDR_VIRT
u32 edma_4_chan_saved[4] = {12,13,14,15};
u32 edma_4_chan_mask = 0xF000;
#else
u32 edma_4_chan_saved[4] = {8,9,10,11};
u32 edma_4_chan_mask = 0xF00;
#endif

EDMAC_CPU_REG_STRU edma_16_int_mask_bak;
EDMAC_CPU_REG_STRU edma_4_int_mask_bak;

struct EDMAC_TRANSFER_CONFIG_STRU edma_16_chan_cfg[3];
...
/*******************************************************************************
  ...
  函数描述:     系统下电前被调用，用于保存EDMA硬件的状态，做其他一些必要的处理
                ...
                要确保该核使用的edma既不在idle，又没有中断。然后才能睡下去
  输入参数:     dev：表示当前设备
  输出参数:     无
  返回值:       成功：0
                失败：非0
...
专为gu 临时版本: edma16 - channel 2 6 7
                 edma 4 - channel 0 1 2 3
*******************************************************************************/
static s32 bsp_edma_suspend(struct dpm_device *dev)
{
    s32 edma16_ret = -1;
    struct edma_reg_struct * edma16_reg_str = 0;

    s32 edma4_ret = -1;
    struct edma_reg_struct * edma4_reg_str = 0;

    edma16_reg_str = g_edma_drv_info.edma_base_addr[0];
    edma16_ret = (s32)(edma16_reg_str->ulChannelState & edma_16_chan_mask);
    if((edma16_ret)!=0)
        {return EDMA_FAIL;}

    edma_16_int_mask_bak = edma16_reg_str->stCpuXReg[EDMAC_DT_ARM];
    edma_16_chan_cfg[0] = edma16_reg_str->stTransferConfig[2];
    edma_16_chan_cfg[1] = edma16_reg_str->stTransferConfig[6];
    edma_16_chan_cfg[2] = edma16_reg_str->stTransferConfig[7];
#ifdef HI_EDMA_CH4_REGBASE_ADDR_VIRT
    edma4_reg_str = g_edma_drv_info.edma_base_addr[0];
    edma4_ret = (s32)(edma4_reg_str->ulChannelState & edma_4_chan_mask);
    if((edma4_ret)!=0)
        {return EDMA_FAIL;}
    edma_4_int_mask_bak = edma4_reg_str->stCpuXReg[EDMAC_DT_ARM];
    edma_4_chan_cfg[0] = edma4_reg_str->stTransferConfig[12];
    edma_4_chan_cfg[1] = edma4_reg_str->stTransferConfig[13];
    edma_4_chan_cfg[2] = edma4_reg_str->stTransferConfig[14];
    edma_4_chan_cfg[3] = edma4_reg_str->stTransferConfig[15];
#else
    edma4_reg_str = g_edma_drv_info.edma_base_addr[0];
    edma4_ret = (s32)(edma4_reg_str->ulChannelState & edma_4_chan_mask);
    if((edma4_ret)!=0)
        {return EDMA_FAIL;}
    edma_4_int_mask_bak = edma4_reg_str->stCpuXReg[EDMAC_DT_ARM];
    edma_4_chan_cfg[0] = edma4_reg_str->stTransferConfig[8];
    edma_4_chan_cfg[1] = edma4_reg_str->stTransferConfig[9];
    edma_4_chan_cfg[2] = edma4_reg_str->stTransferConfig[10];
    edma_4_chan_cfg[3] = edma4_reg_str->stTransferConfig[11];
#endif
    return EDMA_SUCCESS;
}
...
/*******************************************************************************
  ...
  返回值:       成功：0
                失败：非0
*******************************************************************************/
static s32 bsp_edma_resume(struct dpm_device *dev)
{
    struct edma_reg_struct * edma16_reg_str = 0;
    struct edma_reg_struct * edma4_reg_str = 0;

    edma16_reg_str = g_edma_drv_info.edma_base_addr[0];
    edma16_reg_str->stCpuXReg[EDMAC_DT_ARM].ulIntTC1Mask |= edma_16_int_mask_bak.ulIntTC1Mask&edma_16_chan_mask;
    edma16_reg_str->stCpuXReg[EDMAC_DT_ARM].ulIntTC2Mask |= edma_16_int_mask_bak.ulIntTC2Mask&edma_16_chan_mask;
    edma16_reg_str->stCpuXReg[EDMAC_DT_ARM].ulIntErr1Mask |= edma_16_int_mask_bak.ulIntErr1Mask&edma_16_chan_mask;
    edma16_reg_str->stCpuXReg[EDMAC_DT_ARM].ulIntErr2Mask |= edma_16_int_mask_bak.ulIntErr2Mask&edma_16_chan_mask;
    edma16_reg_str->stCpuXReg[EDMAC_DT_ARM].ulIntErr3Mask |= edma_16_int_mask_bak.ulIntErr3Mask&edma_16_chan_mask;
    edma16_reg_str->stTransferConfig[2] = edma_16_chan_cfg[0];
    edma16_reg_str->stTransferConfig[6] = edma_16_chan_cfg[1];
    edma16_reg_str->stTransferConfig[7] = edma_16_chan_cfg[2];
#ifdef HI_EDMA_CH4_REGBASE_ADDR_VIRT
    edma4_reg_str = g_edma_drv_info.edma_base_addr[0];
    ...
    edma4_reg_str->stTransferConfig[12] = edma_4_chan_cfg[0];
    edma4_reg_str->stTransferConfig[13] = edma_4_chan_cfg[1];
    edma4_reg_str->stTransferConfig[14] = edma_4_chan_cfg[2];
    edma4_reg_str->stTransferConfig[15] = edma_4_chan_cfg[3];
#else
    edma4_reg_str = g_edma_drv_info.edma_base_addr[0];
    edma4_reg_str->stCpuXReg[EDMAC_DT_ARM].ulIntTC1Mask |= edma_4_int_mask_bak.ulIntTC1Mask&edma_4_chan_mask;
    edma4_reg_str->stCpuXReg[EDMAC_DT_ARM].ulIntTC2Mask |= edma_4_int_mask_bak.ulIntTC2Mask&edma_4_chan_mask;
    ...
    edma4_reg_str->stCpuXReg[EDMAC_DT_ARM].ulIntErr2Mask |= edma_4_int_mask_bak.ulIntErr2Mask&edma_4_chan_mask;
    edma4_reg_str->stCpuXReg[EDMAC_DT_ARM].ulIntErr3Mask |= edma_4_int_mask_bak.ulIntErr3Mask&edma_4_chan_mask;
    edma4_reg_str->stTransferConfig[8] = edma_4_chan_cfg[0];
    edma4_reg_str->stTransferConfig[9] = edma_4_chan_cfg[1];
    edma4_reg_str->stTransferConfig[10] = edma_4_chan_cfg[2];
    edma4_reg_str->stTransferConfig[11] = edma_4_chan_cfg[3];
#endif
...
struct dpm_device g_edma_dpm_device = {
...
/*******************************************************************************
  ...
  返回值:       成功：0
                失败：非0
*******************************************************************************/
static s32  bsp_edma_probe(void)
{
    s32 ret = -1;
...
#ifdef CONFIG_CCORE_PM
    ret = bsp_device_pm_add(&g_edma_dpm_device);
    if(ret)
    {
        hiedmac_trace(BSP_LOG_LEVEL_ERROR, "fail to add edma dpm device\r\n");
        return ret;
    }
#endif
    ...
    if(request_irq(INT_LVL_EDMAC, (irq_handler_t)edma_isr, 0, "Balong_V7R2_EDMAC", EDMA_CH16_ID))
    {
    ...
    if(BOARD_TYPE_GUL == bsp_get_board_mode_type())
	{
        ...
        hiedmac_trace(BSP_LOG_LEVEL_DEBUG,"edmac request irq = %d\n", INT_LVL_EDMAC_CH4);
        if(request_irq(INT_LVL_EDMAC_CH4, (irq_handler_t)edma_isr, 0, "Balong_V7R2_EDMAC_CH4", (void*)EDMA_CH4_ID))
        {
