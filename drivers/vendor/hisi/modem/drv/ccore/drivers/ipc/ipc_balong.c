/*lint --e{525,537,607,713,732,752,830}*/
#ifdef __KERNEL__
...
static int get_ipc_int_stat(struct dpm_device *dev)
{
	...
	if(g_ipc_resume_int_stat)
	{
        ...
        bsp_pm_log((unsigned int)PM_OM_CIPC, sizeof(g_ipc_resume_int_stat), (void *)&g_ipc_resume_int_stat);
		if(g_ipc_resume_int_stat & (((unsigned int)1) << IPC_CCPU_INT_SRC_ICC))
		{
			icc_wakeup_flag_set();
