...
-e64 -e732 -e740 -e515 -e516 -e10 -e40 -e409 -e63 -e129 -e52 -e19 -e718 -e801 -e745 -e746 -e830 -e762 -e578
-e601 -e132 -e26 -e18 -e49 -e101 -e102 -e713 -esym(628,*)
*/
...
BSP_VOID BSP_IPF_MEM(BSP_VOID)
{
    ...
    IPF_PRINT("IPF_ULBD_MEM_ADDR   0x%x\t\t%u\n", (unsigned long *)IPF_ULBD_MEM_ADDR, IPF_ULBD_MEM_SIZE);		 
    IPF_PRINT("IPF_ULRD_MEM_ADDR   0x%x\t\t%u\n", (unsigned long *)IPF_ULRD_MEM_ADDR, IPF_ULRD_MEM_SIZE);		 
    IPF_PRINT("IPF_DLBD_MEM_ADDR   0x%x\t\t%u\n", (unsigned long *)IPF_DLBD_MEM_ADDR, IPF_DLBD_MEM_SIZE);		 
    IPF_PRINT("IPF_DLRD_MEM_ADDR   0x%x\t\t%u\n", (unsigned long *)IPF_DLRD_MEM_ADDR, IPF_DLRD_MEM_SIZE);		 
    IPF_PRINT("IPF_DLCD_MEM_ADDR   0x%x\t\t%u\n", (unsigned long *)IPF_DLCD_MEM_ADDR, IPF_DLCD_MEM_SIZE);		 
    IPF_PRINT("IPF_INIT_ADDR  	   0x%x\t\t%u\n", (unsigned long *)IPF_INIT_ADDR, IPF_INIT_SIZE);				 
    IPF_PRINT("IPF_DEBUG_INFO_ADDR 0x%x\t\t%u\n", (unsigned long *)IPF_DEBUG_INFO_ADDR, IPF_DEBUG_INFO_SIZE);
}
...
BSP_S32 IPF_FilterList_Init(BSP_VOID)
{
    ...
    {
        /* coverity[alloc_fn] */
        current = (IPF_ID_S*)malloc(sizeof(IPF_ID_S));/* [false alarm]:fortify disable */
        if(NULL == current)
        {
...
{
	if (g_stIPFDebugInfo->ipf_wakeup_ccore_intr_count) {
		...
		g_stIPFDebugInfo->ipf_wakeup_ccore_intr_count = 0;
	}
	return;
}
...
static int ipf_ul_resume(struct dpm_device *dev)
{
	...
	status = ipf_readl(HI_IPF_REGBASE_ADDR_VIRT + HI_IPF_INT0_OFFSET); 
	if (status & (IPF_UL_RPT_INT0 \
					| IPF_UL_TIMEOUT_INT0 \
					| IPF_UL_ADQ0_EPTY_INT0 \
					| IPF_UL_ADQ1_EPTY_INT0)) {
		g_stIPFDebugInfo->ipf_wakeup_ccore_intr_count++;
	}
    g_stIPFDebugInfo->ipf_ccore_resume_count++;
	return IPF_SUCCESS;
}
