...
{    
	...
	{	
		...
		if(0 != s32Ret)
		{
		    bsp_trace(BSP_LOG_LEVEL_FATAL, BSP_MODU_IPF,"Comp error \n");
		    bsp_trace(BSP_LOG_LEVEL_FATAL, BSP_MODU_IPF,"g_pu8STUlData\n");
		    bsp_trace(BSP_LOG_LEVEL_FATAL, BSP_MODU_IPF,"===================================\n");
		    p=(BSP_U32 *)g_pu8STUlData;
		    for(i=0; i < 16; i+=4)
		    {
		        bsp_trace(BSP_LOG_LEVEL_FATAL, BSP_MODU_IPF,"0x%08x    0x%08x    0x%08x    0x%08x\n", *(p+i), *(p+i+1), *(p+i+2), *(p+i+3));
		    }
		    bsp_trace(BSP_LOG_LEVEL_FATAL, BSP_MODU_IPF,"0x%08x    0x%08x\n", *(p+i), *(p+i+1));
		    bsp_trace(BSP_LOG_LEVEL_FATAL, BSP_MODU_IPF,"stRd[%d].u32OutPtr\n",j);
            bsp_trace(BSP_LOG_LEVEL_FATAL, BSP_MODU_IPF,"===================================\n");
            bsp_trace(BSP_LOG_LEVEL_FATAL, BSP_MODU_IPF,"stRd[%d].u32OutPtr =%x\n",j,stRd[j].u32OutPtr);
