...
MAILBOX_LOCAL void mailbox_show_general(struct mb_cfg *cfg)
{
    ...
    printf("0x%08x,  0x%08x,  0x%08x,   0x%08x, %04d"RT,(unsigned int)cfg->butt_id, 
                    (unsigned int)cfg->head_addr, (unsigned int)cfg->data_addr, 
                    (unsigned int)cfg->data_size, (unsigned int)cfg->int_src);
    printf(("Head information:"RT));

    /*打印此邮箱的邮箱头内容。*/
    printf("Head Front: 0x%x (0x%08x)"RT,  (unsigned int)pBoxHead->ulFront,
                (unsigned int)(cfg->data_addr + (pBoxHead->ulFront * sizeof(unsigned long))));
    printf("Head Rear: 0x%x (0x%08x)"RT, (unsigned int)pBoxHead->ulRear,
                (unsigned int)(cfg->data_addr + (pBoxHead->ulRear * sizeof(unsigned long))));
    printf("Head Frontslice: 0x%x"RT, (unsigned int)pBoxHead->ulFrontslice);
    printf("Head Rearslice: 0x%x"RT,  (unsigned int)pBoxHead->ulRearslice);
    printf((":-------------------------------------------------------------:"RT));
                ...
                int clear)
{
    ...
    do {
        ...
        printf("%02d   ,0x%-8x  ,0x%-8x   ,0x%-8x  ,0x%-8x(%d)"RT,
    ...
    if (mb->local_id == mailbox_get_dst_id(channel)) {
        /*如果是接收通道*/
        /*打印通道方向*/
        printf(("Receive Channel"RT));
    }

    /*打印通道方向*/
    if (mb->local_id == mailbox_get_src_id(channel)) {
        printf("Send Channel: sem id(0x%08x)"RT,(unsigned int)mbuf->mutex);
    } else if (mb->local_id == mailbox_get_dst_id(channel)) {
        printf("Receive Channel: sem id(0x%08x)"RT,(unsigned int)mbuf->mutex);
    ...
    printf("Peak Traffic: %d%%, Peak: 0x%x, Total: 0x%x"RT,
                ...
                unsigned int    show_flag)
{
    ...
    if (MAILBOX_SHOW_ALL & show_flag) {
        ...
        if (MAILBOX_NULL != mbuf) {
            ...
            printf(("mail box show global channel config:"RT));
            ...
            printf(("Latest error log track:"RT));
            printf(("error num,   line num,   file name"RT));
            do {
