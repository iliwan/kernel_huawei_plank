...
/*PC模拟板侧ST的收邮件通知函数*/
extern unsigned int mailbox_send_msg(unsigned int, void *, unsigned int);
extern unsigned int mailbox_reg_msg_cb(unsigned int, mb_msg_cb, void *);
extern unsigned int mailbox_read_msg_data(void *, char *, unsigned int *);
extern void st_mailbox_notify(unsigned long MailCode);/*lint --e{752}*/
/*lint --e{628}*/

/*lint -save -e737 -e701 -e438*/
...
{
    unsigned long temp_value;/*lint --e{830}*/
    unsigned long bound_width = 0;

    if ((fls((int)total_size) + fls((int)Second_To_Millisecond)) < MAILBOX_BOARDST_BOUNDWIDTH_MUL_LIMIT) {
        temp_value = total_size * Second_To_Millisecond;
        if ((fls((int)temp_value) + fls((int)Second_To_Millisecond))  < MAILBOX_BOARDST_BOUNDWIDTH_MUL_LIMIT) {
...
#ifndef _DRV_LLT_
        time_us = (unsigned long)BSP_GetSliceValue();/*lint --e{628,746}*/
#else
        time_us--;
#endif
...
char* tm_mailbox_msg_pack(unsigned long rcode, unsigned long size)
{
    ...
    int pack_size = (int)sizeof(struct mb_st_msg) + (int)size;/*lint --e{713,737}*/
                ...
                unsigned int		mail_len)
{
    ...
    (void)mailbox_read_msg_data(mail_handle, buff, &mail_len);/*lint --e{628,746}*/
    /*lint -restore*/
    /* [false alarm]:fortify */
    test_handle = (struct mb_st_msg *)buff;/* [false alarm]:fortify */
    /* [false alarm]:fortify */
...
)
{
    ...
    /* coverity[secure_coding] *//*lint -e{713}*/
    sprintf(TaskName,"tm_mailbox_msg_multi_send(%d)",(unsigned int)task_num);
    return (unsigned long)taskSpawn(TaskName, (int)priority, 0, 1024, entry,
            (int)send_code, (int)recv_code, (int)send_len, (int)send_num, (int)delay_tick, (int)delay_num, (int)try_times, (int)mb_cb, 0, 0); /* [false alarm]:fortify */
                ...
                struct tm_mb_cb * mb_cb)
{
    ...
    while (msg_num) {
        ...
        /*lint --e{732}*/
        mb_cb->prob_slice = mb_cb->prob_slice ? mb_cb->prob_slice : ((unsigned long)tm_mailbox_get_time_stamp());
        RetVal =  mailbox_send_msg(
            (unsigned int)send_code, pData, send_size + sizeof(struct mb_st_msg)); /* [false alarm]:fortify *//*lint !e732*/
        ...
        if ((0 != delay_tick)  && (0 != delay_num) && (0 == msg_num%delay_num)) { /* [false alarm]:fortify */
            taskDelay((int)delay_tick); /* [false alarm]:fortify */
        }
...
)
{
    ...
    /*lint -save -e64*/
    mb_cb->finish_sema  = semBCreate(SEM_Q_PRIORITY, (int)SEM_EMPTY);
    mb_cb->sync_sema  = semBCreate(SEM_Q_PRIORITY, (int)SEM_EMPTY);
    /*lint -restore*/
    mb_cb->check_ret = MAILBOX_CRIT_RET_START;

    /*注册消息响应接收回调*/
    /*lint -save -e628 -e718 -e746*/
    (void)mailbox_reg_msg_cb(recv_code, tm_mailbox_msg_cb, mb_cb);/*lint --e{628,746}*/
    /*lint -restore*/
...
{
    return (long)memcpy(dest, src, (u32)dest_len);
}

long ccore_memcmp(const void * src, int src_len, const void * dest, int dest_len)
{
    return (long)memcmp(src, dest, (u32)dest_len);
}
