...
u32* sg_pAllocSizeTbl = (u32*)((unsigned long)MEM_CTX_ADDR + MEM_CTX_RESERVED);
MEM_ALLOC_INFO* sg_pIccAllocInfo = (MEM_ALLOC_INFO*)((unsigned long)MEM_CTX_ADDR + sizeof(sg_AllocListSize) + MEM_CTX_RESERVED);
u32* g_mem_init_mark = (u32*)MEM_CTX_ADDR;
...
((u32)(ptr) >= AXI_MEM_ADDR && (u32)(ptr) < AXI_MEM_ADDR+AXI_MEM_SIZE)


#define MEM_LOCAL_LOCK() \
do{\
    sg_localLockKey = intLock();\
}while(0)

#define MEM_LOCAL_UNLOCK() intUnlock(sg_localLockKey)

#define MEM_SPIN_LOCK() \
do{\
    MEM_LOCAL_LOCK();\
        bsp_ipc_spin_lock(IPC_SEM_MEM);\
}while(0)

#define MEM_SPIN_UNLOCK() \
do{\
        bsp_ipc_spin_unlock(IPC_SEM_MEM);\
    MEM_LOCAL_UNLOCK();\
}while(0)
...
#define MEM_INVALID_CACHE(ptr, size) (void)cacheInvalidate(DATA_CACHE, ptr, size)
...
STATIC s32 bsp_mem_dump_block(u32 u32Addr)
{
    ...
    UNUSED(bBlockInvalid);
    
    if (NULL == (void *)u32Addr)
    {
        printf("can't dump the block, the input ptr is NULL\n");
        return ERROR;
    }
    ...
    if (MEM_MAGIC_NUM != MEM_ITEM_MAGIC(u32Addr))
    {
        bBlockInvalid = TRUE;
        UNUSED(bBlockInvalid);
        printf("The Block:0x%x may Invalid!\n", u32Addr);
    }
...
s32 bsp_mem_dump_alloc_list(u32 u32Size, u32 u32PoolType)
{
    ...
    printf("dump alloc list end, num:%d\n", u32Num);
    return 0;
}
...
STATIC s32 bsp_mem_scan(u32 u32PoolType)
{
    ...
    while(u32CurScan < u32EndAddr)
    {
        ...
        if(!s32GetChar)
        {
            printf("error! please input a valid char\n");
        }
        if ('q' == s32GetChar)
        {
            break;
