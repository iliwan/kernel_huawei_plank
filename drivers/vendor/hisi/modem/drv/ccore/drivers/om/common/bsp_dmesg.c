...
static int emit_log_char_time(char c)
{
	...
	if (new_text_line)
	{
		...
		if (printk_time) {
			...
			char tbuf[50], *tp;/* [false alarm]:fortify */
			int tlen;/* [false alarm]:fortify */
			u32 t;/* [false alarm]:fortify */

			t = om_timer_get();/* [false alarm]:fortify */
...
//	(void)semTake (g_dmesg_lock, WAIT_FOREVER);
	while(buff < buffer + len)
	{
		if(printk_time)
		{
			(void)emit_log_char_time(*buff);/* [false alarm]:fortify */
		}
		else
		{
