...
/*****************************************************************************
  2 全局变量定义
*****************************************************************************/
bsp_om_global_s     g_om_global_info ;
...
u32 bsp_om_socp_chan_init(void)
{
    ...
    /* 申请RD空间 */
    g_bsp_om_socp_chan_info.rd_buf = (u32)bsp_om_alloc(BSP_OM_CODER_SRC_RDSIZE,&(g_bsp_om_socp_chan_info.rd_buf_phy));/* [false alarm]:fortify */
    ...
    channle_stu.u32DestChanID = (u32)g_bsp_om_socp_chan_info.en_dst_chan_id;    /* [false alarm]:fortify *//*  目标通道ID */
    ...
    channle_stu.sCoderSetSrcBuf.u32InputStart  = (u32)g_bsp_om_socp_chan_info.bd_buf;           /*  输入通道起始地址 */
    channle_stu.sCoderSetSrcBuf.u32InputEnd    = (u32)((g_bsp_om_socp_chan_info.bd_buf
                                                                + g_bsp_om_socp_chan_info.bd_buf_len)-1);   /*  输入通道结束地址 */
    channle_stu.sCoderSetSrcBuf.u32RDStart     = (u32)(g_bsp_om_socp_chan_info.rd_buf);                              /* RD buffer起始地址 */
    channle_stu.sCoderSetSrcBuf.u32RDEnd       = (u32)((g_bsp_om_socp_chan_info.rd_buf
...
void bsp_socp_chan_enable(void)
{
    if(g_bsp_om_socp_chan_info.init_state == BSP_OM_SOCP_CHAN_INIT_SUCC)
    {
        bsp_socp_start(BSP_OM_CCPU_CODER_SRC_CHAN);
    }
}
...
u32 bsp_om_get_buf(bsp_om_buf_type_e buf_type,u32 get_buf_len)
{
    u32 free_size = 0;
    u32 return_addr = 0;
    u32 read_ptr = 0;
...
u32 bsp_om_send_log_buf(u32 buf_type,u32 packet_len)
{
    ...
    while(g_bsp_log_buf_info.valible_node_num > 0)
    {
