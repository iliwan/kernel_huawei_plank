...
void sysview_trace_packet(u8 *trace_buf,u32 buf_size,bsp_sysview_type_e data_type)
{
    ...
    {
        bsp_om_header->om_id            = (BSP_STRU_ID_28_31_GROUP_BSP << 28) | (BSP_STRU_ID_16_23_BSP_TASK << 16)|CMD_BSP_SYSVIEW_IND_CCORE;
        sysview_info_header->sn              = g_om_global_info.task_info_sn++;
    }

    else if(BSP_SYSVIEW_INT_LOCK_INFO== data_type)
    {
        bsp_om_header->om_id            = (BSP_STRU_ID_28_31_GROUP_BSP << 28) | (BSP_STRU_ID_16_23_BSP_INT_LOCK << 16)|CMD_BSP_SYSVIEW_IND_CCORE;
        sysview_info_header->sn              = g_om_global_info.int_lock_info_sn++;
    }
    else if(BSP_SYSVIEW_MEM_TRACE== data_type)
    {
        bsp_om_header->om_id            = (BSP_STRU_ID_28_31_GROUP_BSP << 28) | (BSP_STRU_ID_16_23_BSP_MEM<< 16)|CMD_BSP_SYSVIEW_IND_CCORE;
        sysview_info_header->sn              = g_om_global_info.mem_info_sn++;
    }
    else if(BSP_SYSVIEW_CPU_INFO== data_type)
    {
        bsp_om_header->om_id            = (BSP_STRU_ID_28_31_GROUP_BSP << 28) | (BSP_STRU_ID_16_23_BSP_CPU<< 16)|CMD_BSP_SYSVIEW_IND_CCORE;
...
u32 sysview_get_all_task_name(void *p_task_stru,u32 param_len)
{
    u32 i;
    u32 task_num = 0;
    s32 taskIdList[BSP_OM_MAX_TASK_NUM] = {0};
    ...
    if(p_task_stru == NULL)
    {
     ...
     /*获得当前所有任务ID*/
    task_num = (UINT32)taskIdListGet(taskIdList, BSP_OM_MAX_TASK_NUM);
    if (task_num > BSP_OM_MAX_TASK_NUM)
    {
        task_num = BSP_OM_MAX_TASK_NUM;
    }
    ...
    {
        ptask_stru[i].task_id = (u32)taskIdList[i];
        strncpy((char *)ptask_stru[i].name, (const char *)taskName(taskIdList[i]), BSP_SYSVIEW_TASK_NAME_LEN);
    }
...
u32 sys_view_init(void)
{
    ...
    {
        bsp_om_init_list_head((struct list_head *)&(g_om_mem_trace.module_trace[mod].alloc_trace_list.p_list));
    }
