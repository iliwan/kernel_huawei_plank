...
/*lint --e{944,506,525,64,119,101,132,537,958,438,830,752,762,713,732,539,774} */

/*lint -save -e534 -e550 -e650 -e715 -e734 -e774 -e826 -e831 -e944 -e506 -e830 -e539 -e774*/
/* ecs */
#include <vxWorks.h>
#include <logLib.h>
#include <semLib.h>
...
/************************************************************
***********INITIALISATION AND POWERUP ROUTINES**************
************************************************************/
...
extern void sci_class_switch_try(void);
...
/******************************************************************************
 * Function:      pl131_registers_init
 * Description:   Initialising smartcard register
 * Input:
 * Output:
 * Return:
 * Others:
...
STATUS pl131_registers_init(SCI_STATE_STRU * parapState)
{
    ...
    else if (((pState->sSetupParams.ClockFreq / PL131_DEF_CARDFREQ) / 2 - 1) <= apBIT_MASK(PL131_REGCLKICC_DATA))
    {
        pState->sSetupParams.CardFreq = PL131_DEF_CARDFREQ;
        InvalidParam = TRUE;
    }
    /*fail*/
    else
                             ...
                             unsigned char * const  pATRBuffer)
{
    ...
    if (nActiveT > SCI_MAX_USIM_CARD_DETECT_RETRY_CNT)
    {
        ...
        else
        {
            ...
            SCI_RECORD_EXCGBL(SCI_EVENT_ERROR_NOCARD, parapState->eCurrentState, omTimerGet());
    ...
    if (apBIT_GET(pBase->RegCtrl0, PL131_REGCTRL0_CLKDIS) == 1)
    {
        ...
        sci_print_debug("The bit of CLKDIS is set %ld\n", apBIT_GET(pBase->RegCtrl0, PL131_REGCTRL0_CLKDIS), 0, 0, 0, 0, 0);
    }
...
STATUS appl131_data_transfer (SCI_STATE_STRU *       parapState,
                             ...
                             unsigned char * const         pDataBuffer,
                             u32                 DataLength)
{
    /*Create pointers to the instance data block and the register base*/
    SCI_STATE_STRU * const pState = parapState;
    ...
    SCI_RECORD_EVENT(SCI_EVENT_LOW_POWER_EXIT, pState->eCurrentState, __LINE__);
    ...
    if (eDirection == apPL131_TX_TO_CARD)
    {
        ...
        if (lClkCurState & 0x1)
        {
          ...
          /*     使用20ms则不会出现误差，为确保正常，使用100ms*/
            if(BSP_OK !=semTake(g_SCIClockStartSem, 10))/* [false alarm]:*/
            {
        ...
        if (!apBIT_GET(pBase->RegDMACtrl, PL131_REGDMA_TX))
        {
            ...
            if(g_sci_card_hpd_status == CARD_HPD_STATUS_LEAVE)
            {
                ...
                BIT_SET(pBase->RegIntMask, PL131_REGINTMASK_TXTIDE, 0);       /*disable TX interrupt*/
                BIT_SET(pBase->RegTide, PL131_REGTIDE_TX, 0);
                return apERR_PL131_NOTREADY;
            }
            else
            {
...
void   pl131_atr_process (SCI_STATE_STRU * parapState, u32 CharATR)
{
    ...
    if (pState->sATRParams.Grouping == PL131_ATR_GROUPDONE)
    {
                   ...
                   || (pState->sATRParams.NumHist == 1))))
        {
            ...
            /*Guard time*/
            if (pState->sATRParams.ChGuard == 255)
            {
                pBase->RegChGuard = 0;
            }
                               ...
                               u32           ATRCharacter)
{
    ...
    else
    {
        ...
        case 0xB3:
        {
            if (pState->eProtocol == apPL131_PROTOCOL_T1_BLOCK)
            {
                pState->sSetupParams.CharacterTime = (1UL << (ATRCharacter & 0x0F)) + 11;
                pState->sSetupParams.BlockTime = ((1UL << ((ATRCharacter & 0xF0) >> 4)) * 960 + 11);
            }
...
/*lint -restore +e534 +e550 +e650 +e715 +e734 +e774 +e826 +e831 +e944 +e506 +e830 +e539*/
