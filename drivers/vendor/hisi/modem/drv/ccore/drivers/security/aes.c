...
//#include "aes_alg.h"
/*lint --e{451,571,703,732,734, 830}*/
...
int16 crypto_aes_init(crypto_aes *cipher, uint8 keymaterial[32], int16 keybits, int32 mode, uint8 iv[16])
{
	...
	crypto_aes_set_key(&(cipher->aes_ctx), keymaterial, (uint32)keybits, 0);
		...
		case MODE_CFB1:
			...
			for (i = num_blocks; i > 0; i--)
			{
				...
				for (k = 0; k < 128; k++) {
					...
					iv[15] = (iv[15] << 1) | ((output[(uint32)k >> 3] >> (7 - (k & 7))) & 1);
				}
				output += 16;
				input  += 16;
			}
		...
		case MODE_CFB1:
			...
			for (i = num_blocks; i > 0; i--)
			{
				...
				for (k = 0; k < 128; k++)
				{
					...
					iv[15] = (iv[15] << 1) | ((input[(uint32)k >> 3] >> (7 - (k & 7))) & 1);
					output[(uint32)k >> 3] ^= (block[0] & 0x80U) >> (k & 7);
				}
				output += 16;
				input += 16;
