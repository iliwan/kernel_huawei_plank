...
BSP_S32 SocpAllocEncChan1(BSP_U32 u32ChanId)
{
    ...
    EncSrcAttr.sCoderSetSrcBuf.u32InputStart = (BSP_U32)p;
    EncSrcAttr.sCoderSetSrcBuf.u32InputEnd = (BSP_U32)p + len - 1;
    ret = bsp_socp_coder_set_src_chan(u32ChanId, &EncSrcAttr);
    if(BSP_OK != ret)
    {
        printf("alloc chan failed, return error(0x%x)\n", ret);
        free(g_pSocpBuf[--g_SocpBufCnt]);
        return -1;
    }
    return u32ChanId;
}
...
BSP_S32 SocpAllocEncChan2(BSP_U32 u32ChanId)
{
    ...
    EncSrcAttr.u32BypassEn = BSP_FALSE;
    EncSrcAttr.u32DestChanID = SOCP_CODER_DEST_CHAN_0;
    EncSrcAttr.sCoderSetSrcBuf.u32InputStart = (BSP_U32)p1;
    EncSrcAttr.sCoderSetSrcBuf.u32InputEnd = (BSP_U32)p1 + len - 1;
    EncSrcAttr.sCoderSetSrcBuf.u32RDStart = (BSP_U32)p2;
    EncSrcAttr.sCoderSetSrcBuf.u32RDEnd = (BSP_U32)p2 + len2 - 1;
...
BSP_S32 SocpStAllocEncSrc(SOCP_ST_CASE_S *pCase)
{
    ...
    {
        pRd = (BSP_U8*)cacheDmaMalloc(pCase->u32RDSize);
        if(BSP_NULL == pRd)
        {
    ...
    EncSrcAttr.sCoderSetSrcBuf.u32InputStart = (BSP_U32)p;
    EncSrcAttr.sCoderSetSrcBuf.u32InputEnd = (BSP_U32)p + pCase->u32InputSize - 1;
    if(pCase->eMode == SOCP_ENCSRC_CHNMODE_LIST)
    {
        EncSrcAttr.sCoderSetSrcBuf.u32RDStart = (BSP_U32)pRd;
        EncSrcAttr.sCoderSetSrcBuf.u32RDEnd = (BSP_U32)pRd + pCase->u32RDSize - 1;
...
BSP_S32 SOCP_ST_ENCODE_006_START()
{
    ...
    if(OK != osl_task_init(task_name, 100, 0x1000, socp_encode_task005, &g_stCase[0], &(g_stCase[0].TaskId)))
    {
        return ERROR;
    }
...
BSP_S32 SOCP_ST_ENCODE_006_STOP()
{
    ...
    cacheDmaFree(g_pSocpBuf[--g_SocpBufCnt]);    //任务中打包所用内存
    cacheDmaFree(g_pSocpBuf[--g_SocpBufCnt]);      //编码源块数据内存
    if(g_stCase[0].eMode == SOCP_ENCSRC_CHNMODE_LIST)
    {
        cacheDmaFree(g_pSocpBuf[--g_SocpBufCnt]);  //编码RD buffer内存
    } 
    /*释放任务*/
    (BSP_VOID)taskDelete(g_stCase[0].TaskId);
...
BSP_S32 SOCP_ST_ENCODE_007_START()
{
    ...
    if(OK != osl_task_init(task_name, 100, 0x1000, socp_encode_task006, &g_stCase[1], &(g_stCase[1].TaskId)))
    {
        return ERROR;
    }
...
BSP_S32 SOCP_ST_ENCODE_007_STOP()
{
    ...
    /*释放任务*/
    (BSP_VOID)taskDelete(g_stCase[1].TaskId);
...
BSP_S32 SOCP_ST_ENCODE_012_START()
{
    ...
    if(OK != osl_task_init(task_name, 100, 0x1000, socp_encode_task001, pCase, &pCase->TaskId))
    {
        return ERROR;
    }
...
BSP_S32 SOCP_ST_ENCODE_014_START()
{
    ...
    sprintf(task_name,"socpTest%02d",10);
    if(OK != osl_task_init(task_name, 100, 0x1000, socp_encode_task009, pCase, &pCase->TaskId))
    {
        return ERROR;
    }
...
BSP_S32 SOCP_ST_ENCODE_015_START()
{
    ...
    sprintf(task_name,"socpTest%02d",10);
    if(OK != osl_task_init(task_name, 100, 0x1000, socp_encode_task010, pCase, &pCase->TaskId))
    {
        return ERROR;
    }
...
BSP_S32 SOCP_ST_ENCODE_016_START()
{
    ...
    sprintf(task_name,"socpTest%02d",10);
    if(OK != osl_task_init(task_name, 100, 0x1000, socp_encode_task011, &g_stCase[0], &(g_stCase[0].TaskId)))
    {
        return ERROR;
    }
...
BSP_S32 SOCP_ST_ENCODE_020_START()
{
    ...
    if(OK != osl_task_init(task_name, 100, 0x1000, socp_encode_task001, pCase4, &(pCase4->TaskId)))
    {
        return ERROR;
    }
    
    sprintf(task_name,"socpTest%03d",195);
    if(OK != osl_task_init(task_name, 100, 0x1000, socp_encode_task003, pCase5, &(pCase5->TaskId)))
    {
        return ERROR;
    }  
    
    sprintf(task_name,"socpTest%03d",196);
    if(OK != osl_task_init(task_name, 100, 0x1000, socp_encode_task001, pCase6, &(pCase6->TaskId)))
    {
        return ERROR;
    } 
    
    sprintf(task_name,"socpTest%03d",197);
    if(OK != osl_task_init(task_name, 100, 0x1000, socp_encode_task003, pCase7, &(pCase7->TaskId)))
    {
        return ERROR;
    } 
...
BSP_S32 SOCP_ST_ENCODE_020_STOP()
{
   /*停止通道*/
    ...
    (BSP_VOID)taskDelete(g_stCase[11].TaskId);
    (BSP_VOID)taskDelete(g_stCase[12].TaskId);
    (BSP_VOID)taskDelete(g_stCase[13].TaskId);
    (BSP_VOID)taskDelete(g_stCase[14].TaskId);
...
BSP_S32 SOCP_ST_ENCODE_DCORE_STRESS()
{
    ...
    sprintf(task_name,"socpTest%02d",230);
    if(OK != osl_task_init(task_name, 100, 0x1000, socp_encode_dcore_task_stress, &g_stCase[0], &(g_stCase[0].TaskId)))
    {
        return ERROR;
    } 
...
BSP_S32 SOCP_ST_DEBUG_TEST(void)
{
    ...
    buffer = (u8 *)cacheDmaMalloc((u32)0x2000);
