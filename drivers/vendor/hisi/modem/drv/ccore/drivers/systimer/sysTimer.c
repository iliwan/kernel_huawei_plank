...
void sysClkInt (void)
{
    ...
    if(readValue != 0)
    {
        ...
        /*清中断*/
	BSP_REG_READ(SYSTIMER_BASE_ADDR, CLK_REGOFF_CLEAR, readValue);
...
void sysClkEnable (void)
{
    ...
    if (!sysClkRunning)
	{
        ...
        do
        {
            BSP_REG_READ(SYSTIMER_BASE_ADDR,CLK_REGOFF_CTRL, readValueTmp);
...
STATUS sysClkRateSet
    ...
    {
        logMsg("sysClkRateSet err, ticksPerSecond: %d \n",ticksPerSecond,0,0,0,0,0);/* [false alarm]:误报 dead code*/
	    return ERROR;
    }
