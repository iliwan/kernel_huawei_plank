...
void test_tcxo_pm_log(void)
{
	tcxo_pm_om_log((unsigned int)TCXO_ON, PWC_COMM_MODE_GSM);
	tcxo_pm_om_log((unsigned int)TCXO_OFF, PWC_COMM_MODE_WCDMA);
	tcxo_pm_om_log((unsigned int)TCXO_TIMER_START, PWC_COMM_MODE_LTE);
	tcxo_pm_om_log((unsigned int)TCXO_TIMER_START, PWC_COMM_MODE_TDS);
	tcxo_pm_om_log((unsigned int)TCXO_STATUS, (unsigned int)PWC_COMM_MODEM_1);
	tcxo_pm_om_log((unsigned int)TCXO_TIMER_HANDER, (unsigned int)PWC_COMM_MODEM_1);
	tcxo_pm_om_log((unsigned int)TCXO_FIRST_ON, (unsigned int)20);
	tcxo_pm_om_log((unsigned int)TCXO_PM_STATUS,g_stTcxoVoteMap[PWC_COMM_MODEM_1]);
}
...
void bsp_tcxo1_enable_first(void)
{
    ...
    if(regvalue & ((unsigned int)0x1 << 7)){
		...
		if(tmp & ((unsigned int)0x1 << 13))
		{
			...
			tcxo_pm_om_log((unsigned int)TCXO_FIRST_ON, (unsigned int)t2-t1);
...
int bsp_tcxo_getstatus(PWC_COMM_MODEM_E enModemId)
{
	...
	tcxo_pm_om_log((unsigned int)TCXO_STATUS, enModemId);
...
int bsp_tcxo_enable(PWC_COMM_MODEM_E  enModemId, PWC_COMM_MODE_E enModeType)
{
    ...
    if(g_stTcxoVoteMap[tcxoId] == 0)
    {
        /*open TCXO1*/
        bsp_tcxo1_enable();
		tcxo_pm_om_log((unsigned int)TCXO_ON, (unsigned int)enModeType);
    }
...
int bsp_tcxo_disable(PWC_COMM_MODEM_E  enModemId, PWC_COMM_MODE_E enModeType)
{
        ...
        if(0 == g_stTcxoVoteMap[tcxoId])
        {
                bsp_tcxo1_disable();
                tcxo_soft_timer[tcxoId].tcxo_status = PWRCTRL_COMM_OFF;
 				tcxo_pm_om_log((unsigned int)TCXO_OFF, (unsigned int)enModeType);
        }
out:
        local_irq_restore(flags);
        return ret;
...
int bsp_tcxo_timer_start(PWC_COMM_MODEM_E  enModemId, PWC_COMM_MODE_E enModeType, PWC_TCXO_FUNCPTR routine, int arg, unsigned int timerValue)
{
	...
	/* coverity[unsigned_compare] */
		...
		tcxo_pm_om_log((unsigned int)TCXO_TIMER_START, (unsigned int)enModeType);
		ret = DRV_TIMER_START(TIMER_CCPU_DRX1_STABLE_ID,(FUNCPTR_1)tcxo_timer_handle,(int)enModemId,timerValue,TIMER_ONCE_COUNT,TIMER_UNIT_MS);
		if(ret){
