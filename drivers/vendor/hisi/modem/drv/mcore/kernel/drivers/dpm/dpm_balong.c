...
static s32 enter_suspend_and_resume(u32 pm_msg)
{
	...
	if(PMSG_SUSPEND==pm_msg){
		...
		(void)balongv7r2_m_uart_suspend();
		...
		temp_time1 = bsp_get_slice_value();
        if(ret != 0)
        {
            pm_resume_flag = 1;
	...
	tem_resume:
		tem_protect_resume();
		temp_time2 = bsp_get_slice_value();
		temp_time1 = get_timer_slice_delta(temp_time1,temp_time2);
		temp_time1 = (temp_time1*1000000)>>15;
		if(temp_time1<500)
			udelay(500-temp_time1);
