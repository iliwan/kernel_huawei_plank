...
/*****************************************************************************
* 功能  : buck3上电后恢复RF RESETN状态,只给v711提供
*****************************************************************************/
#ifdef RF_RESETN_MSAVE
static void rf_resetn_resume(void)
{
	writel(1,HI_CTU_BASE_ADDR + RFIC0_RSTN_OFFSET);
}
#endif
...
/* sdio bug has fixed in v711, don't need config pmu for sdio */
static inline s32 dpm_suspend_buck3off_v711(void)
{
    return 0;
}
...
step12:
	...
	writel(0x10000, CM3_CRG_BASE_ADDR + HI_CRG_SRSTEN2_OFFSET);
	writel(0x10000, CM3_CRG_BASE_ADDR + HI_CRG_SRSTDIS2_OFFSET);
	/* 13 */
