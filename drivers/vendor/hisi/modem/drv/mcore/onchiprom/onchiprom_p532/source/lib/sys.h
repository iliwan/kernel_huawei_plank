...
{\
    ...
    writel(readl(TIMER0_REGOFF_CTRL)&(~0x80), TIMER0_REGOFF_CTRL);/* disable */\
    writel(0xffffffff, TIMER0_REGOFF_LOAD); /* init-count */\
    writel(readl(TIMER0_REGOFF_CTRL)|(0x82), TIMER0_REGOFF_CTRL);  /* enable, free-running */\
}

#define TIMER0_DEINIT()\
{\
    writel(readl(TIMER0_REGOFF_CTRL) & (~0xa0), TIMER0_REGOFF_CTRL);       /* disable */\
}
