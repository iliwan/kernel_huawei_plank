...
void usb3_release(void)
{
    ...
    /* ref_ssp_en */
    reg = (volatile uint32_t *)(INTEGRATOR_SC_BASE+0x434);
    value = usb3_rd32(reg);
    value |= (uint32_t)1 << 31;
    usb3_wr32(reg, value);
    ...
    reg = (volatile uint32_t *)(INTEGRATOR_PERI_BASE+0xC14);
    value = usb3_rd32(reg);
    ...
    reg = (volatile uint32_t *)BC_CTL_BASE;
    usb3_wr32(reg, 2);

    /* release controller and PHY */
    reg = (volatile uint32_t *)(INTEGRATOR_PERI_BASE+0x070);
    value = usb3_rd32(reg);
    value |= (uint32_t)(0x1F << 21);
    usb3_wr32(reg, value);
...
void usb3_driver_init(void)
{
    ...
    usb3_dev->base = (volatile uint8_t *)USB_REG_BASE;
