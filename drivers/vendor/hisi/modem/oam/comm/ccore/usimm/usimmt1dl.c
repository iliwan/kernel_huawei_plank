...
)
{
    ...
    lSCIResult = DRV_USIMMSCI_SND_DATA(ulRBlkLen, pucRBlk);
...
VOS_UINT32  USIMM_T1DLSendIBlock(VOS_UINT8 *pucData, VOS_UINT32 ulLen, VOS_UINT32 ulSeqNum)
{
    ...
    lSCIResult = DRV_USIMMSCI_SND_DATA(ulIBlkLen, pucIBlk);
    OM_RecordInfoEnd(VOS_EXC_DUMP_MEM_NUM_1);
...
VOS_UINT32  USIMM_T1DLSendSBlock(USIMM_T1_S_BLOCK_STRU *pstSBlk)
{
    ...
    lSCIResult = DRV_USIMMSCI_SND_DATA(ulSBlkLen, pucSBlk);
    OM_RecordInfoEnd(VOS_EXC_DUMP_MEM_NUM_1);
...
)
{
    ...
    switch (enSBlkType)
    {
        ...
        case USIMM_T1_S_BLOCK_WTX_REQ:
            ...
            if (USIMM_SCI_SUCCESS == DRV_USIMMSCI_SETBWT(ucData))
            {
                ulResult = VOS_OK;
            }
            else
...
)
{
    ...
    else
    {
        ...
        if (USIMM_T1_DEFAULT_WTX != g_stUsimmT1CtrlCtx.ulBWTTimes)
        {
            ...
            if (USIMM_SCI_SUCCESS != DRV_USIMMSCI_SETBWT(USIMM_T1_DEFAULT_WTX))
            {
                /* 如果调用底软API出错，打印错误 */
...
{
    VOS_INT32       lSCIResult;
    VOS_UINT8       aucRecData[USIMM_T1_BLOCK_MAX_SIZE + 1];    /* 增加一字节保护 */
    VOS_UINT32      ulRecBlockLen;
    ...
    lSCIResult = DRV_USIMMSCI_BLK_RCV(aucRecData, (VOS_UINT *)&ulRecBlockLen);
    OM_RecordInfoEnd(VOS_EXC_DUMP_MEM_NUM_1);
