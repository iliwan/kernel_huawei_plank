...
VOS_UINT32 VOS_RegisterAwakeFun(Msg_Fun_Type pfnHook)
{
    ...
    g_pfnVosAwakeFunHook = pfnHook; /* [false alarm]: 屏蔽Fortify错误 */
...
VOS_VOID VOS_ExecuteAwakeFun(MsgBlock *pstMsgCtrlBlk)
{
    ...
    /* coverity[unreachable] */
    if ( VOS_NULL_PTR == g_pfnVosAwakeFunHook ) /* [false alarm]: 屏蔽Fortify错误 */
    {
        return;
    }

    (g_pfnVosAwakeFunHook)(pstMsgCtrlBlk); /* [false alarm]: 屏蔽Fortify错误 */
...
VOS_UINT32 VOS_CheckInterrupt( VOS_VOID )
{
    ...
    return (VOS_UINT32)in_interrupt(); /* [false alarm]: 屏蔽Fortify错误 */
#endif
    return VOS_FALSE;
}
...

{
                                        ...
                                        VOS_NORMAL_PRIORITY_MSG, ulPid ) )
    {
        LogPrint3("# V_SendMsg Error,queue full,Tx Pid %d Rx Pid %d Name 0x%x.\n",
            (int)(pMsgCtrlBlk->ulSenderPid), (int)ulPid, /* [false alarm]: 屏蔽Fortify错误 */
        ...
        pstDumpMsgInfo->ulSendPid = pMsgCtrlBlk->ulSenderPid; /* [false alarm]: 屏蔽Fortify错误 */
...
{
    MsgBlock *pstMsgBlock = (MsgBlock*)( ulAddrress + VOS_MSG_BLK_HEAD_LEN ); /* [false alarm]: 屏蔽Fortify错误 */
