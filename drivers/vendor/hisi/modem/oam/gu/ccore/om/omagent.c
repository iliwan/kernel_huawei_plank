...
extern OM_VERSION_QRY_DEBUG_INFO_STRU  g_stVerQryDbgInfo;
...
/*****************************************************************************
 Prototype       : OM_VerQryDbgInfoReqProc
 Description     :
 Input           : pMsg - 指向消息地址
 Output          : None
 Return Value    : VOS_VOID
 ...
 History         : ---
    ...
    Modification : Created function
 *****************************************************************************/
VOS_VOID OM_VerQryDbgInfoReqProc(MsgBlock *pMsg)
{
    OM_VERSION_QRY_DEBUG_INFO_CNF_STRU *pstOmVerQryDbgInfo;

    /* 构造消息发送 */
    pstOmVerQryDbgInfo = (OM_VERSION_QRY_DEBUG_INFO_CNF_STRU *)VOS_AllocMsg(CCPU_PID_OMAGENT,
                                                                            sizeof(OM_VERSION_QRY_DEBUG_INFO_CNF_STRU) - VOS_MSG_HEAD_LENGTH);

    if (VOS_NULL_PTR == pstOmVerQryDbgInfo)
    {
        return;
    }

    pstOmVerQryDbgInfo->ulReceiverPid  = ACPU_PID_OMAGENT;
    pstOmVerQryDbgInfo->usPrimId       = OM_VERSION_DEBUG_INFO_CNF;

    VOS_MemCpy(&pstOmVerQryDbgInfo->stVerDebugInfo, &g_stVerQryDbgInfo, sizeof(OM_VERSION_QRY_DEBUG_INFO_STRU));

    (VOS_VOID)VOS_SendMsg(CCPU_PID_OMAGENT, pstOmVerQryDbgInfo);
...
OM_AGENT_ITME_STRU g_astOmAgentProcTbl[] =
{
    ...
    {OM_VERSION_DEBUG_INFO_REQ, OM_VerQryDbgInfoReqProc},
};
