...
#endif
    pstPcb->cwnd                        = (BST_UINT16)( pstPcb->mss * (BST_UINT16)2  );
    pstPcb->ssthresh                    = (BST_UINT16)( pstPcb->mss * (BST_UINT16)10 );
    pstPcb->connected                   = connected;
    TCP_EVENT_CONNECTED( pstPcb, ERR_OK, ret );
...
/***
    pstPcb，当MODEM侧发起时，不从该PCB读取属性，而是读取PreProc记录的Last值，这是只用于读取端口号
    ...
    BST_IP_SKT_PROPTY_STRU     *pstCurrentProperty )
{
    ...
    /*如果端口已经挂起，那么说明同步由Modem发起，从预处理取走*/
    ...
    BST_DBG_LOG2( "BST_IP_ApiGetTcpPcbProperty ulTxByte: %d ulRxByte: %d",\
...
BST_VOID BST_IP_ApiDropTcpPcb( struct tcp_pcb *pstPcb, BST_UINT32 ulBeRmv )
{
