...
)
{
    ...
    for ( ulStep = 0; ulStep < pstNvimCfgDPlmnNPlmnInfo->usDplmnListNum; ulStep++ )
    {
        PS_MEM_CPY(stNvimPlmn.aucSimPlmn,
            pstNvimCfgDPlmnNPlmnInfo->aucDPlmnList + (ulStep * (NAS_MML_PLMN_WITH_RAT_UNIT_LEN + 1)), NAS_MML_SIM_PLMN_ID_LEN);

        /* 转换PLMN ID为MMC内部的格式并存储 */
        NAS_MMC_ConvertSimPlmnToNasPLMN(&stNvimPlmn, &stTempPlmn);

        /* 存储PLMN对应的RAT */
        usSimRat    = (VOS_UINT16)(pstNvimCfgDPlmnNPlmnInfo->aucDPlmnList[(ulStep * (NAS_MML_PLMN_WITH_RAT_UNIT_LEN + 1)) + NAS_MML_PLMN_WITH_RAT_FIRST_RAT_OFFSET] << NAS_MML_OCTET_MOVE_EIGHT_BITS)
                                | pstNvimCfgDPlmnNPlmnInfo->aucDPlmnList[(ulStep * (NAS_MML_PLMN_WITH_RAT_UNIT_LEN + 1)) + NAS_MML_PLMN_WITH_RAT_SECOND_RAT_OFFSET];

        enRegDomain = (VOS_UINT8)(pstNvimCfgDPlmnNPlmnInfo->aucDPlmnList[ulStep * (NAS_MML_PLMN_WITH_RAT_UNIT_LEN + 1) + NAS_MML_PLMN_WITH_RAT_UNIT_LEN]);
    ...
    {
        PS_MEM_CPY(stNvimPlmn.aucSimPlmn, pstNvimCfgDPlmnNPlmnInfo->aucNPlmnList + (ulStep * (NAS_MML_PLMN_WITH_RAT_UNIT_LEN + 1)), NAS_MML_SIM_PLMN_ID_LEN);

        /* 转换PLMN ID为MMC内部的格式并存储 */
        NAS_MMC_ConvertSimPlmnToNasPLMN(&stNvimPlmn, &stTempPlmn);

        /* 存储PLMN对应的RAT */
        usSimRat = (VOS_UINT16)(pstNvimCfgDPlmnNPlmnInfo->aucNPlmnList[(ulStep * (NAS_MML_PLMN_WITH_RAT_UNIT_LEN + 1)) + NAS_MML_PLMN_WITH_RAT_FIRST_RAT_OFFSET] << NAS_MML_OCTET_MOVE_EIGHT_BITS)
                     | pstNvimCfgDPlmnNPlmnInfo->aucNPlmnList[(ulStep * (NAS_MML_PLMN_WITH_RAT_UNIT_LEN + 1)) + NAS_MML_PLMN_WITH_RAT_SECOND_RAT_OFFSET];

        enRegDomain = (VOS_UINT8)(pstNvimCfgDPlmnNPlmnInfo->aucNPlmnList[ulStep * (NAS_MML_PLMN_WITH_RAT_UNIT_LEN + 1) + NAS_MML_PLMN_WITH_RAT_UNIT_LEN]);
     ...
     && (NAS_MML_REG_FAIL_CAUSE_NULL == enPsRegCause))
    {
        ...
        if (NAS_MML_REG_FAIL_CAUSE_IMSI_UNKNOWN_IN_HLR != enCsRegCause)
        {
              ...
              && (VOS_TRUE == ulIsNeedDisableL))
            {
                ...
                if (VOS_TRUE == NAS_MML_IsNeedAddPlmnInDisabledPlmnWithValidPeriodList(&stPlmnWithRat))
                {
...
VOS_VOID  NAS_MMC_UpdateDPlmnNPlmnList(
...
)
 {
    ...
    {
        PS_MEM_MOVE(&pstDestPlmnIdList[1], &pstDestPlmnIdList[0],
                               (*pulDestPlmnNum) * sizeof(NAS_MMC_SIM_PLMN_WITH_REG_DOMAIN_STRU));

        pstDestPlmnIdList[0].stSimPlmnWithRat.stPlmnId.ulMcc    = pstCurrPlmn->ulMcc;
        pstDestPlmnIdList[0].stSimPlmnWithRat.stPlmnId.ulMnc    = pstCurrPlmn->ulMnc;
        pstDestPlmnIdList[0].stSimPlmnWithRat.usSimRat    = usSimRat;
        pstDestPlmnIdList[0].enRegDomain = enRegDomain;
        (*pulDestPlmnNum)++;
    }
    else
    {
        PS_MEM_MOVE(&pstDestPlmnIdList[1], &pstDestPlmnIdList[0],
                               (NAS_MMC_MAX_CFG_DPLMN_NUM - 1) * sizeof(NAS_MMC_SIM_PLMN_WITH_REG_DOMAIN_STRU));
