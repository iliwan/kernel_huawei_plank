...
VOS_VOID NAS_MMC_SndMmaSysInfo( VOS_VOID )
{
    ...
    {
        if ((VOS_FALSE == NAS_MML_GetHplmnInEplmnDisplayHomeFlg())
         || (VOS_FALSE == ulIsHplmnInEplmnList))
        {
...
VOS_VOID  NAS_MMC_SndMmaMsgLmmCellSignInfoInd(
    MMA_MMC_SCELL_MEAS_REPORT_TYPE_UN  *punMeasReportType,
    MMA_MMC_LMM_CELL_SIGN_INFO_STRU     *pstRssiValueInfo
    )
{
    MMC_MMA_LMM_CELL_SIGN_INFO_REPORT_IND_STRU      *pstSndMsg            = VOS_NULL_PTR;                               /* 定义原语类型指针                         */

    pstSndMsg = (MMC_MMA_LMM_CELL_SIGN_INFO_REPORT_IND_STRU *)PS_ALLOC_MSG(WUEPS_PID_MMC,
                         sizeof(MMC_MMA_LMM_CELL_SIGN_INFO_REPORT_IND_STRU) - VOS_MSG_HEAD_LENGTH);

    if( VOS_NULL_PTR == pstSndMsg )
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndMmaMsgLmmCellSignInfoInd():ERROR:Memory Alloc Error for pstRssiInfoInd ");
        return;
    }

    PS_MEM_SET(((VOS_UINT8*)pstSndMsg) + VOS_MSG_HEAD_LENGTH, 0, sizeof(MMC_MMA_LMM_CELL_SIGN_INFO_REPORT_IND_STRU) - VOS_MSG_HEAD_LENGTH);

    /* 按MMC_MMA_LMM_RSSI_IND_STRU消息封装 */
    ...
    /* 填充消息头 */
    pstSndMsg->MsgHeader.ulMsgName         = ID_MMC_MMA_LMM_CELL_SIGN_INFO_REPORT_IND;
    ...
    pstSndMsg->MsgHeader.ulLength          = sizeof(MMC_MMA_LMM_CELL_SIGN_INFO_REPORT_IND_STRU) - VOS_MSG_HEAD_LENGTH;

    if (VOS_OK != PS_SEND_MSG(WUEPS_PID_MMC, pstSndMsg))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "PS_SEND_MSG:Error: Failed in NAS_MMC_SndMmaMsgLmmCellSignInfoInd()");
    }
...
VOS_VOID NAS_MMC_SndMmaPsServiceConnStatusInd(
    VOS_UINT8                           ucPsServiceConnStatusFlag
)
{
    MMC_MMA_PS_SERVICE_CONN_STATUS_IND_STRU    *pstPsServiceConnStatusInd = VOS_NULL_PTR;

    pstPsServiceConnStatusInd = (MMC_MMA_PS_SERVICE_CONN_STATUS_IND_STRU *)PS_ALLOC_MSG(WUEPS_PID_MMC,
                        sizeof(MMC_MMA_PS_SERVICE_CONN_STATUS_IND_STRU) - VOS_MSG_HEAD_LENGTH);
    ...
    /* 填充消息头 */
    pstPsServiceConnStatusInd->stMsgHeader.ulMsgName         = ID_MMC_MMA_PS_SERVICE_CONN_STATUS_IND;
    ...
    pstPsServiceConnStatusInd->stMsgHeader.ulLength          = sizeof(MMC_MMA_PS_SERVICE_CONN_STATUS_IND_STRU) - VOS_MSG_HEAD_LENGTH;

    /* 填充消息*/
    pstPsServiceConnStatusInd->ucPsServiceConnStatusFlag   = ucPsServiceConnStatusFlag;

    if (VOS_OK != PS_SEND_MSG(WUEPS_PID_MMC, pstPsServiceConnStatusInd))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "PS_SEND_MSG:Error: Failed in NAS_MMC_SndMmaPsServiceConnStatusInd()");
    }

    return;
}

VOS_VOID NAS_MMC_SndMmaSrchedPlmnInfo(const MMC_MMA_SRCHED_PLMN_INFO_IND_STRU  *pstMmcSrchedPlmnINfo)
{
    MMC_MMA_SRCHED_PLMN_INFO_IND_STRU  *pstSrchedPlmnInfoInd = VOS_NULL_PTR;

    pstSrchedPlmnInfoInd = (MMC_MMA_SRCHED_PLMN_INFO_IND_STRU *)PS_ALLOC_MSG(WUEPS_PID_MMC,
                            sizeof(MMC_MMA_SRCHED_PLMN_INFO_IND_STRU) - VOS_MSG_HEAD_LENGTH);

    if (VOS_NULL_PTR == pstSrchedPlmnInfoInd)
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndMmaSrchedPlmnInfo():ERROR:Memory Alloc Error for pstSrchedPlmnInfoInd ");
        return;
    }
    
    /* 填充ID_MMC_MMA_SRCHED_PLMN_INFO_IND消息 */
    PS_MEM_CPY(pstSrchedPlmnInfoInd, 
               pstMmcSrchedPlmnINfo,
               sizeof(MMC_MMA_SRCHED_PLMN_INFO_IND_STRU));

    /* 填充消息头 */
    pstSrchedPlmnInfoInd->stMsgHeader.ulMsgName         = ID_MMC_MMA_SRCHED_PLMN_INFO_IND;
    ...
    pstSrchedPlmnInfoInd->stMsgHeader.ulReceiverPid     = WUEPS_PID_MMA;
    pstSrchedPlmnInfoInd->stMsgHeader.ulLength          = sizeof(MMC_MMA_SRCHED_PLMN_INFO_IND_STRU) - VOS_MSG_HEAD_LENGTH;

    if (VOS_OK != PS_SEND_MSG(WUEPS_PID_MMC, pstSrchedPlmnInfoInd))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "PS_SEND_MSG:Error: Failed in NAS_MMC_SndMmaSrchedPlmnInfo()");
    }
}
