...
VOS_VOID NAS_RABM_StopGprsRaT1Timer(VOS_VOID)
{
    if (RABM_ROUTING_AREA_UPDATE_T1_ON == NAS_RABM_GetRauT1Flg())
    {
        ...
        if (VOS_OK != NAS_RabmStopTimer(RABM_TIMER_NAME_COMMON, RABM_TIMER_PARA_ROUT_AREA_UPDATE_T1))
        {
            NAS_WARNING_LOG(WUEPS_PID_RABM,
                "NAS_RABM_StopGprsRaT1Timer: Stop rout.area.update.T1 timer failed.");
        }
    }
}
VOS_VOID NAS_RABM_Stop3GTo2GT3Timer(VOS_VOID)
{
    if (RABM_3G_TO_2G_T3_ON == NAS_RABM_GetW2GT3Flg())
    {
        ...
        if (VOS_OK != NAS_RabmStopTimer( RABM_TIMER_NAME_COMMON, RABM_TIMER_PARA_3_TO_2_T3))
        {
            NAS_WARNING_LOG(WUEPS_PID_RABM,
                "NAS_RABM_Stop3GTo2GT3Timer: Stop abm.sys.3To2.T3 timer failed.");
        }
    }
}
