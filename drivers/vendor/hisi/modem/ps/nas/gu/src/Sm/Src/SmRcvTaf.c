...
)
{
    ...
    {                                                                           /* 为MS发起的请求(Cr映射的NSAPI无效)        */
        ...
        {                                                                       /* PDP context存在,并且PDP address和APN相同 */
            SM_SndTafSmPdpActivateRej(ucCr, SM_TAF_CAUSE_SM_DUPLICATE, VOS_NULL_PTR);   /* 回拒绝消息                               */
        }
        else
        {
...
)
{
    ...
    if ( 0xff == ucCntxtIndex )
    {                                                                           /* 状态为SM_PDP_INACTIVE                    */
        PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_WARNING, "SM_RcvNwReqPdpCnxtActMsg_S02:WARNING:0xff == ucCntxtIndex ");
        return;
    }
    ...
    {                                                                           /* 状态为SM_PDP_NW_ACTIVE                   */
        ...
        PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_NORMAL, "SM_RcvTafPdpCnxtActRejRsp:NORMAL:SM state: SM_PDP_INACTIVE ==> SM_PDP_INACTIVE");

        PS_LOG1(WUEPS_PID_SM, VOS_NULL, PS_PRINT_NORMAL, "SM_RcvTafPdpCnxtActRejRsp:NORMAL: Destroy SM entity(index):", ucCntxtIndex);
    ...
    {
        PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_WARNING, "SM_RcvTafPdpCnxtActRejRsp:WARNING:APP->SM: Message type not compatible with the state  !");
    }
    return;
}
    ...
    else
    {
        ...
        {                                                                       /* 包含Tear down indicator IE并且 requested */
            ...
            {                                                                   /* Nsapi列表                                */
                ...
                PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_NORMAL, "SM_RcvTafPdpCnxtDeActReq:NORMAL: SM state = SM_PDP_INACTIVE");
...
)
{
    ...
    {                                                                           /* 状态不为SM_PDP_INACTIVE                  */
        ...
        {                                                                       /* 当前的状态为SM_PDP_NW_MODIFY             */
            ...
            PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_NORMAL, "SM_RcvTafPdpModifyRsp:NORMAL:SM state: SM_PDP_NW_MODIFY ==> SM_PDP_ACTIVE");

            NAS_SM_SndGmmPdpModifyInd(ucCntxtIndex + SM_NSAPI_OFFSET);
        }
        else
        {
            PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_WARNING, "SM_RcvTafPdpModifyRsp:WARNING:APP->SM: Message type not compatible with the state  !");
        }
    }
    return;
}
...
)
{
    ...
    if (PS_PTL_VER_PRE_R99 == ue_revision)
    {
        ...
        PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_WARNING, "SM_RcvTafPdpCntxtModifyReq:WARNING:ue_revision is PS_PTL_VER_PRE_R99,modify PDP req is rejected!");
        return;
    }
    ...
    {                                                                           /* 状态不为SM_PDP_INACTIVE                  */
        ...
        {                                                                       /* 当前的状态为SM_PDP_ACTIVE                */
            ...
            PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_NORMAL, "SM_RcvTafPdpCntxtModifyReq:NORMAL:SM state: SM_PDP_ACTIVE ==> SM_PDP_MODIFY_PENDING");
        }
        else if (SM_PDP_MODIFY_PENDING
                 == g_SmEntity.aPdpCntxtList[ucCntxtIndex].ucState )
        {

            PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_WARNING, "SM_RcvTafPdpCntxtModifyReq:WARNING:APP->SM: Message type not compatible with the state  !");
        }
        else if (SM_PDP_NW_MODIFY
                 == g_SmEntity.aPdpCntxtList[ucCntxtIndex].ucState )
        {
            SM_SndTafSmPdpModifyRej(SM_TAF_CAUSE_SM_MODIFY_COLLISION,
                g_SmEntity.aPdpCntxtList[ucCntxtIndex].ucCr);                                        /* 回拒绝消息                               */
        }
        else
        {
...
)
{
    ...
    if (PS_PTL_VER_PRE_R99 == ue_revision)
    {
        ...
        PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_WARNING, "SM_RcvTafPdpCnxtActSecReq:WARNING:ue_revision is PS_PTL_VER_PRE_R99,active secondary PDP req is rejected!");
        return;
    }
    ...
    if ( 0xff != ucCntxtIndex )
    {                                                                           /* 状态不为SM_PDP_INACTIVE                  */
        SM_SndTafSmPdpActSecRej(ucCr, SM_TAF_CAUSE_SM_INVALID_NSAPI);             /* 回拒绝消息                               */
    }
    else
    {
        ...
        {                                                                       /* ucLinkCntxtIndex状态为已经激活           */
            ...
            case SM_PDP_ACTIVE_PENDING:
                ...
                break;
            default:
                SM_SndTafSmPdpActSecRej(ucCr, SM_TAF_CAUSE_SM_NW_UKNOWN_PDP_CONTEXT);         /* 回拒绝消息                               */
                break;
        }
    }
    return;
...
)
{
    ...
    for ( i = 0; i < SM_MAX_NSAPI_NUM; i++ )
    {
        ...
        {
            ucCntxtIndex = (VOS_UINT8)i;      /*A32D00461,zzy,pclint*/                                              /* 获取实体的索引                           */
            break;
        }
    }
    ...
    for ( i = 0x80; i <= (SM_MAX_TI_NUM - 1); i++ )
    {
        ...
        {                                                                       /* 申请没有使用的TI                         */
            ucTi = (VOS_UINT8)i;        /*A32D00461,zzy,pclint*/                                                   /* 获取TI                                   */
            ucTiFlg = SM_TRUE;
            break;
        }
    }
    ...
    {                                                                           /* 申请 实体,PDP Addr,Ti成功                */
        ...
        PS_LOG1(WUEPS_PID_SM, VOS_NULL, PS_PRINT_NORMAL, "SM_RcvTafPdpCnxtActSecReq_S00:NORMAL: Create SM entity(index):", ucCntxtIndex);
        ...
        PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_NORMAL, "SM_RcvTafPdpCnxtActSecReq_S00:NORMAL:SM state: SM_PDP_INACTIVE ==> SM_PDP_ACTIVE_PENDING");
        NAS_SM_SndGmmPdpStatusEx();
...
)
{
    ...
    {                                                                           /* 申请 实体,PDP Addr,Ti成功                */
        ...
        PS_LOG1(WUEPS_PID_SM, VOS_NULL, PS_PRINT_NORMAL, "SM_RcvTafPdpCnxtActReq_S00:NORMAL:Create SM entity(index):", ucCntxtIndex);
        ...
        PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_NORMAL, "SM_RcvTafPdpCnxtActReq_S00:NORMAL:SM state: SM_PDP_INACTIVE ==> SM_PDP_ACTIVE_PENDING ");
...
)
{
    ...
    {                                                                           /* 申请 PDP Addr成功                        */
        ...
        PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_NORMAL, "SM_RcvTafPdpCnxtActReq_S05:NORMAL:SM state: SM_PDP_INACTIVE ==> SM_PDP_ACTIVE_PENDING");
        SM_SndPdpActReqMsg(pstMsg, ucCntxtIndex);                               /* 发送消息REQUEST                          */
        g_SmEntity.aPdpCntxtList[ucCntxtIndex].TimerInfo.ucExpireTimes = 0;     /* 超时次数清0                              */
        SM_TimerStart(ucCntxtIndex,SM_T3380_LENGTH,SM_TIMER_TYPE_T3380);        /* 启动T3380                                */
/*
    ...
    else
    {
        for (ulIndex = 0; ulIndex < SM_MAX_NSAPI_NUM; ulIndex++)
        {
            if (VOS_NULL_PTR == g_SmEntity.aHoldMsg[ulIndex].pHoldRcvMsg)
            {
                ...
                {
                    PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_ERROR, "NAS_SM_RcvTafPdpCnxtActReq: ERROR: ALLOC MSG FAIL");
                    return;
                }
    ...
    else if (SM_PS_REGISTERING == g_SmEntity.ucRegisterFlg)
    {
        for (ulIndex = 0; ulIndex < SM_MAX_NSAPI_NUM; ulIndex++)
        {
            if (VOS_NULL_PTR == g_SmEntity.aHoldMsg[ulIndex].pHoldRcvMsg)
            {
                ...
                if (VOS_NULL_PTR == pHoldMsg)
                {
                    PS_LOG(WUEPS_PID_SM, VOS_NULL, PS_PRINT_ERROR, "NAS_SM_RcvTafPdpCnxtDeActReq: ERROR: ALLOC MSG FAIL");
                    return;
                }
...
)
{
    ...
    else
    {
        ...
        {
            /*lint -e701*/
            if( 0 != ( g_SmPdpAddrApn.usPdpAddrFlg & ( 0x01<<i )))
            /*lint +e701*/
            {                                                                   /* AddrApn存在                              */
