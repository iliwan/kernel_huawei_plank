...
VOS_VOID NAS_SMS_RcvNackMsg(MMSMS_NACK_DATA_IND_STRU *pstNackMsg)
{
    ...
    */
     ...
     || (SMR_MTN_DATA_TYPE_RP_ERR == ucRpType))
    {
        ...
        {
            PS_LOG1(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_INFO, "NAS_SMS_RcvNackMsg: g_SmcCsEnt.SmcMt.ucTi ", g_SmcCsEnt.SmcMt.ucTi);
            PS_LOG1(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_INFO, "NAS_SMS_RcvNackMsg: ucTiValue ", ucTiValue);
            PS_LOG1(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_INFO, "NAS_SMS_RcvNackMsg: ucTiFlag ", ucTiFlag);
            return;
        }
                      ...
                      )
{
    ...
    {                                                                           /* 结果错误                                 */
        ...
        PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_WARNING, "SMC_RcvNwCsMtData:WARNING:SMC_MSG_VALID != ucResult ");
        return;
    }
    ...
    case SMC_DATA_TYPE_CP_DATA:
        ...
        if(g_SmcCsEnt.SmcMt.ucTi != (pucCpdu[0] >> 4))
        {
            if(SMC_MT_WAIT_FOR_CP_ACK == g_SmcCsEnt.SmcMt.ucState)
            {
                ...
                /* 停止timer TC1M */
                SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_RcvNwCsMtData: TimerStop: TC1M" );
                if ( SMS_FALSE == SMS_TimerStop( SMC_TIMER_ID_CS_TC1M_MT ) )
                {
                    SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_RcvNwCsMtData: TimerStop: TC1M failed" );
                }

                g_SmcCsEnt.SmcMt.TimerInfo.ucExpireTimes = 0;                       /* 清除timer超时次数                        */
                g_SmcCsEnt.SmcMt.ucState                 = SMC_MT_MM_CONN_EST;      /* 状态迁移                                 */
            ...
            else
            {
                ...
                SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_RcvNwCsMtData: Invalid Ti,ignore MSG" );
                return ;
            }
        }
        ...
        {                                                                       /* 实体状态是空闲或在等待网侧确认           */
            PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMC_RcvNwCsMtData:NORMAL:SMS state = SMC_MT_WAIT_FOR_RP_ACK");
            g_SmcCsEnt.SmcMt.ucState = SMC_MT_WAIT_FOR_RP_ACK;                  /* 状态迁移                                 */
    ...
    case SMC_DATA_TYPE_CP_ACK:
        ...
        {                                                                       /* 与实体的状态兼容                         */
            ...
            PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMC_RcvNwCsMtData:NORMAL:SMS state = SMC_MT_IDLE");
                     ...
                     )
{
    ...
    {                                                                       /* 与实体的状态兼容                         */
        /* 停止timer TC1M */
        SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_RcvMmEstInd: TimerStop: TC1M" );
        if ( SMS_FALSE == SMS_TimerStop( SMC_TIMER_ID_CS_TC1M_MT ) )
        {
            SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_RcvMmEstInd: TimerStop: TC1M failed" );
        }
        ...
        PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMC_RcvMmEstInd:NORMAL:SMS state = SMC_MT_IDLE");
        g_SmcCsEnt.SmcMt.ucState = SMC_MT_IDLE;                         /* 状态迁移到空闲                           */
    }
    ...
    {                                                                           /* 实体是空闲状态                           */
        ...
        {                                                                       /* 数据类型是CP-DATA                        */
            PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMC_RcvMmEstInd:NORMAL:SMS state = SMC_MT_WAIT_FOR_RP_ACK");
            g_SmcCsEnt.SmcMt.ucState = SMC_MT_WAIT_FOR_RP_ACK;
            g_SmcCsEnt.SmcMt.ucTi    = pucCpdu[0] >> 4;                         /* 存储ti                                   */
            g_SmcCsEnt.SmcMt.ucMr    = pucCpdu[SMC_MSG_HDR_LEN + 2];            /* 存储mr                                   */
                      ...
                      )
{
    ...
    SMS_LOG1(PS_PRINT_NORMAL, "SMC_RcvNwCsMoData: g_SmcCsEnt.SmcMo.ucState.", g_SmcCsEnt.SmcMo.ucState);
    SMS_LOG1(PS_PRINT_NORMAL, "SMC_RcvNwCsMoData: g_SmcPsEnt.SmcMo.ucState.", g_SmcPsEnt.SmcMo.ucState);
    ucResult = SMC_ComNwMsgChk( pucCpdu, ulCpduLen );                           /* 消息检查                                 */
    if(SMC_MSG_VALID != ucResult)
    {                                                                           /* 结果错误                                 */
        if(SMC_CP_ERR_MSG_INVALID != ucResult)
    ...
    SMS_LOG1(PS_PRINT_NORMAL, "SMC_RcvNwCsMoData: event ", pucCpdu[1] );
    switch( pucCpdu[1] )
    {                                                                           /* 消息类型                                 */
    case SMC_DATA_TYPE_CP_DATA:
        ...
        {                                                                       /* 状态正确                                 */
            if (ulCpduLen > (SMC_MSG_HDR_LEN + 1))
            {
                ...
                if(VOS_OK == ulRet )
                {
                    ...
                    {
                        /* 停止timer TC1M */
                        SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_RcvNwCsMoData: TimerStop: TC1M" );
                        if (SMS_FALSE == SMS_TimerStop( SMC_TIMER_ID_CS_TC1M_MO ) )
                        {
                            SMS_LOG( PS_PRINT_WARNING, "SMS:SMC_RcvNwCsMoData: fail to Stop TC1M" );
                        }
                    ...
                    g_SmcCsEnt.SmcMo.ucState    = SMC_MO_WAIT_TO_SND_CP_ACK;            /* 状态迁移                                 */
                }
                else
                {
                    if (SMR_ERR_CAUSE_MSG_PROTOCOL_ERROR != ulRet)
                    {
                        g_SmcCsEnt.SmcMo.ucState = SMC_MO_WAIT_FOR_CP_ACK;            /* 状态迁移                                 */
                    }
                    else
                    {
                        g_SmcCsEnt.SmcMo.ucState = SMC_MO_WAIT_FOR_CP_DATA;            /* 状态迁移                                 */
                    }
    ...
    case SMC_DATA_TYPE_CP_ACK:
        ...
        {                                                                       /* 状态正确                                 */
            ...
            PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMC_RcvNwCsMoData:NORMAL:SMS state = SMC_MO_MM_CONN_EST");
            g_SmcCsEnt.SmcMo.ucState = SMC_MO_MM_CONN_EST;                      /* 状态迁移                                 */
                    ...
                    )
{
    ...
    {                                                                           /* TI错误 忽略消息 返回                     */
        PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_WARNING, "SMC_RcvMmEstCnf:WARNING:g_SmcCsEnt.SmcMo.ucTi != (VOS_UINT8)(pRcvMsg->ulTi) ");
        return;
    }

    if(pRcvMsg->ulTi < 8)
    {                                                                           /* 如果TI小于8                              */
        PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_WARNING, "SMC_RcvMmEstCnf:WARNING:pRcvMsg->ulTi < 8");
        return;
    }

    if(g_SmcCsEnt.SmcMo.HoldRcvMsg.ulMsgLen == 0)
    {                                                                           /* 有缓存的消息                             */
        PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_WARNING, "SMC_RcvMmEstCnf:WARNING:none message to send.");
        return;
    }
    ...
    {
        SMS_LOG( PS_PRINT_WARNING, "SMC_RcvMmEstCnf: TimerStart: TR1M Failed" );
    }
    ...
    /* 启动定时器TC1M */
    SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_RcvMmEstCnf: TimerStart: TC1M" );
    if ( SMS_FALSE == SMS_TimerStart( SMC_TIMER_ID_CS_TC1M_MO ) )
    {
    ...
    PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMC_RcvMmEstCnf:NORMAL:SMS state = SMC_MO_WAIT_FOR_CP_ACK");
    g_SmcCsEnt.SmcMo.ucState = SMC_MO_WAIT_FOR_CP_ACK;                          /* 状态迁移                                 */
    return;
}
...
)
{
    ...
    if(g_SmcCsEnt.SmcMt.ucTi == ucTi)
    {
        ...
        PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMC_ComCsMtErr:NORMAL:SMS state = SMC_MT_IDLE");
        g_SmcCsEnt.SmcMt.ucState = SMC_MT_IDLE;
        SMC_ComCsInitEnt(SMS_TRUE);                                             /* 初始化CS域的实体                         */
...
)
{
    ...
    {                                                                           /* TI相等                                   */
        PS_LOG1(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL,
        ...
        case SMC_MO_MM_CONN_EST:
            PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMC_ComCsMoErr:NORMAL:SMS state = SMC_MO_IDLE");
            g_SmcCsEnt.SmcMo.ucState = SMC_MO_IDLE;                             /* 状态迁移到空闲                           */
            SMC_ComCsInitEnt(SMS_FALSE);                                        /* 初始化CS域的实体                         */
                         ...
                         )
{
    ...
    {                                                                           /* 不是mt过程   即MO过程                    */
        ...
        /* 停止timer TC1M */
        SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_ComCsWaitAckSta: TimerStop: TC1M" );
        if ( SMS_FALSE == SMS_TimerStop( SMC_TIMER_ID_CS_TC1M_MO ) )
        {
            return;
        }
                  ...
                  )
{
    ...
    {
        /* 打印错误信息 */
        SMS_LOG( PS_PRINT_ERROR, "SMS:SMC_SndMmMsg:Error: Send msg fails" );
    }
}
                     ...
                     )
{
    ...
    {                                                                           /* 即MO过程                                 */
        ...
        /* 停止timer TC1M */
        SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_ComCsInitEnt: TimerStop: TC1M" );
        if ( SMS_FALSE == SMS_TimerStop( SMC_TIMER_ID_CS_TC1M_MO ) )
        {
        ...
        g_SmcCsEnt.SmcMo.ucState                 = SMC_MO_IDLE;                 /* 状态转移到空闲                           */
    }
    else
    {
        if( SMC_MT_IDLE != g_SmcCsEnt.SmcMt.ucState )
        ...
        if ( SMS_FALSE == SMS_TimerStop( SMC_TIMER_ID_CS_TC1M_MT ) )
        {
            SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_ComCsInitEnt: fail to stop SMC_TIMER_ID_CS_TC1M_MT." );
        }
