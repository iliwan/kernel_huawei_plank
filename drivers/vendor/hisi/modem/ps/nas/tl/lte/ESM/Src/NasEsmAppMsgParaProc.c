...
#if 0
        return APP_ERR_SM_SET_APN_BEFORE_SET_AUTH;
#endif

    }
#if 0
    if (NAS_ESM_BEARER_TYPE_DEDICATED == pstSdfPara->enBearerCntxtType)
    {
        NAS_ESM_WARN_LOG("NAS_ESM_CheckAppGwAuthInfoOperation:APP->ESM ,Bear type is err!");
...
VOS_VOID NAS_ESM_AppParaSetPco( VOS_VOID *pRcvMsg )
{
    ...
    /*得到参数设置类型*/
    enSetType=pAppPcoMsg->enSetType;
    ...
    {
        /*设置信元标志位*/
        pstSdfPara->bitOpPco = NAS_ESM_OP_TRUE;

        /*从接收消息中获得PCO信息*/
        /*根据ulCid，把PCO信息保存在本地相应结构中*/
        NAS_ESM_MEM_CPY((VOS_VOID*)&(pstSdfPara->stPcoInfo),\
                    (VOS_VOID*)&(pAppPcoMsg->stAppPcoInfo),\
                    sizeof(APP_ESM_PCO_INFO_STRU));
    }
    else
    {

        /*根据ulCid，删除本地相应结构中的PCO信息*/
        pstSdfPara->bitOpPco = NAS_ESM_OP_FALSE;
        NAS_ESM_MEM_SET((VOS_VOID*)&(pstSdfPara->stPcoInfo), 0, \
                    sizeof(APP_ESM_PCO_INFO_STRU));
    }
    #endif
    /*向APP返回设置确认*/
...
VOS_VOID NAS_ESM_AppParaSetPdpManagType( VOS_VOID *pRcvMsg )
{
    ...
    /* 设置成功，则更新参数信息到NV中 */
    if( APP_SUCCESS == pAppBearManagTypeCnf->ulSetRslt)
    {
        NAS_ESM_WritePdpManInfoToNv();
    }
    #endif
#endif
...
#if 0
            /*封装APN参数*/
            ulRslt = NAS_ESM_ApnReverseTransformation(&pAppCnfMsg->stApnInfo,
                                                      &pstSdfPara->stApnInfo);

            if(NAS_ESM_SUCCESS != ulRslt)
            {
                /*打印异常信息*/
                NAS_ESM_ERR_LOG("NAS_ESM_AppParaQueryApn:ERROR:APP->SM ,NAS_ESM_ApnReverseTransformation failed!");
                pAppCnfMsg->ulRslt = APP_ERR_UNSPECIFIED_ERROR;
            }
#endif
...
VOS_VOID NAS_ESM_AppParaQueryPco( VOS_VOID *pRcvMsg )
{
    ...
    else
    {
        ...
        NAS_ESM_MEM_CPY((VOS_VOID*)&(pAppCnfMsg->stAppPcoInfo),\
                    (VOS_VOID*)&(pstSdfPara->stPcoInfo),\
                      sizeof(APP_ESM_PCO_INFO_STRU));
        #endif
    }
...
)
{
    ...
    /*打印进入该函数*/
    NAS_ESM_INFO_LOG("NAS_ESM_AppPDPDynamicQueryMsgProc is entered.");
...
)
{
    ...
    for (ulEpsbId = NAS_ESM_MIN_EPSB_ID; ulEpsbId <= NAS_ESM_MAX_EPSB_ID; ulEpsbId++)
    {
            ...
            NAS_ESM_GetEpsbCntxtInfoAddr(ulEpsbId)->enBearerCntxtState)
        {
            ...
            {
                #if 0
                NAS_ESM_GetEpsbCntxtInfoAddr(ulEpsbId)->stEpsQoSInfo.stQosInfo.ucQCI =
                    NAS_ESM_GetEpsbCntxtInfoAddr(ulEpsbId)->stEpsQoSInfo.ucNwQCI;
                #endif
