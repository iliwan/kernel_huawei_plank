    ...
    if(NAS_ESM_OP_TRUE == pstStateAddr->bitOpAppMsgRecord)
    {
        switch(pstStateAddr->stAppMsgRecord.enAppMsgType)
        {
            ...
            case ID_APP_ESM_PDP_MODIFY_REQ:
                ...
                break;
            default:
                NAS_ESM_WARN_LOG("NAS_ESM_InformAppEpsBearerModSucc:WARNING:No Support App Msg!");
                break;
        }
    }
}
...
VOS_VOID NAS_ESM_DeactBearerAndInformApp(   VOS_UINT32 ulEpsbId )
{
    ...
    if(PS_TRUE == NAS_ESM_IsDefaultEpsBearerType(pstEpsbCntxtInfo->enBearerCntxtType))
    {
        ...
        {
            /* 通知IP模块承载释放 */
            NAS_ESM_SndEsmIpPdpReleaseInd((VOS_UINT8)ulEpsbId);
        }
#endif
...
/*leili修改，该函数没有使用*/
#if 0
VOS_VOID NAS_ESM_LocalReleaseSdf
(
    VOS_UINT32                          ulEpsbId,
    VOS_UINT32                          ulCid
)
{
    ...
    /*打印进入该函数*/
    NAS_ESM_INFO_LOG("NAS_ESM_LocalReleaseSdf is entered.");
    ...
    /* 若此承载没有对应的CID，则退出 */
    if ((NAS_ESM_OP_FALSE == pstEpsbInfo->bitOpCId)
     || (NAS_ESM_NULL == pstEpsbInfo->ulBitCId))
    {
        if (NAS_ESM_BEARER_TYPE_DEDICATED == pstEpsbInfo->enBearerCntxtType)
        {
            /*通知RABM，向RABM发送ID_ESM_ERABM_DEACT_IND*/
            NAS_ESM_SndEsmRabmDeactIndMsg(NAS_ESM_BEARER_ACT_CID_NUM_ONE,
                                         &ulEpsbId);

            NAS_ESM_SndSmEspBearerDeactIndMsg(ulEpsbId);

            /*释放承载资源*/
            NAS_ESM_ClearEsmBearerResource(ulEpsbId);
        ...
        {
            NAS_ESM_WARN_LOG("NAS_ESM_LocalReleaseSdf:Release last Cid of Default bearer .");
        }
    }
    else
    {
        NAS_ESM_INFO_LOG("NAS_ESM_LocalReleaseSdf: SND RABM packetfilter delete!");

        /*向RABM发送ID_ESM_ERABM_MDF_IND*/
        NAS_ESM_SndEsmRabmMdfIndMsg(ulEpsbId);

        NAS_ESM_SndSmEspBearerMdfIndMsg(ulEpsbId);
...
VOS_VOID  NAS_ESM_SaveNwSDFInfo
(
    const  NAS_ESM_NW_MSG_STRU          *pstMsgIE,
    VOS_UINT32                           ulStateTblIndex
)
{
    ...
    pstBearerCntxtInfo = NAS_ESM_GetEpsbCntxtInfoAddr(pstSdfCntxtInfo->ulEpsbId);

    /* 将动态信息拷贝到静态信息中 */
    /* 赋值承载类型 */
    pstSdfPara ->enBearerCntxtType = pstBearerCntxtInfo->enBearerCntxtType;

    /* 赋值Link Cid */
    ulCid = NAS_ESM_GetLinkCidByActiveCid(pstStateAddr->ulCid);
    if (NAS_ESM_ILL_CID == ulCid)
    {
        NAS_ESM_WARN_LOG("NAS_ESM_SaveNwSDFInfo:illegal CID!");
    }
    else
    {
        pstSdfPara->bitOpLinkdCId = NAS_ESM_OP_TRUE;

        pstSdfPara->ulLinkdCId = ulCid;
    }
    ...
    /* 赋值TFT */
    pstSdfPara->ulPfNum = pstSdfCntxtInfo->ulSdfPfNum;
    NAS_ESM_MEM_CPY(  pstSdfPara->astCntxtTftInfo,
                      pstSdfCntxtInfo->astSdfPfInfo,
                      sizeof(APP_ESM_TFT_INFO_STRU)*NAS_ESM_MAX_SDF_PF_NUM);

    /* 赋值App Pf Id */
    NAS_ESM_SetAppPfId(pstSdfPara);
...
)
{
    ...
    for (ulCnt = 0; ulCnt < pstSdfCntxtInfo->ulSdfPfNum; ulCnt++)
    {
        if (NAS_ESM_NULL == pstSdfCntxtInfo->astSdfPfInfo[ulCnt].ucPacketFilterId)
        {
            ...
            {
                NAS_ESM_ERR_LOG("NAS_ESM_AddPacketFilterProc: Get App PfId Failed.");
