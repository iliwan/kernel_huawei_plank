...
)
{
    ...
    if (IMSA_CONN_TYPE_NORMAL == enConnType)
    {
        ...
        {
            IMSA_CallGetCurrentProcessingCallTypeByCallStatus(&enCallType);
            if (IMSA_CALL_TYPE_VOICE == enCallType)
            {
                pstCallCtx->ulIsVoiceMediaPdpReady = IMSA_TRUE;
            }

            if (IMSA_CALL_TYPE_EMC == enCallType)
            {
                pstCallCtx->ulIsEmcMediaPdpReady = IMSA_TRUE;
            }
        }
        if((IMSA_TRUE == IMSA_CONN_HasActiveVideoPdp()) &&
            (IMSA_TRUE == IMSA_CONN_HasActiveVoicePdp()))
        {
            pstCallCtx->ulIsVideoMediaPdpReady = IMSA_TRUE;
        }
    }
    else
    {
...
)
{
    ...
    if (IMSA_CONN_TYPE_NORMAL == enConnType)
    {
        ...
        {
            pstCallCtx->ulIsVoiceMediaPdpReady = IMSA_FALSE;
            pstCallCtx->ulIsVideoMediaPdpReady = IMSA_FALSE;
        }
