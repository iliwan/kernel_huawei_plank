...
VOS_UINT32  NAS_LMM_PreProcInterOptIndMsg( MsgBlock  *pMsg)
{
      ...
      || NAS_EMM_RAT_LRRC_TAILIST_STATUS_PART_IN_FORB_LIST == enRatLrrcTalistStatus)
    {
        ...
        /*如果网络侧4G未开户标识在，则说明UE 4G未开户走的网络侧的标准流程，定时器超时回到L后不再重新disable LTE*/
        if(NAS_EMM_EUTRAN_NOT_ALLOWED == NAS_EMM_GetLteNoSubscribeExtCauseFlag())
        {
              return NAS_LMM_MSG_HANDLED;
        }
        /*不是网络侧带扩展原因值的场景，重新disable LTE*/
