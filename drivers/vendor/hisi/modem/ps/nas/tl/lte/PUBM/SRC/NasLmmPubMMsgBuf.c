...
VOS_VOID  NAS_EMMC_InitBufMsgQue( VOS_VOID )
{
    #if 0
    VOS_UINT32                          ulCont;
    NAS_LMM_FSM_MSG_BUF_STRU             *pstMMCFsmMsgBuf;

    NAS_LMM_PUBM_LOG_NORM(
    "NAS_EMMC_InitBufMsgQue             START INIT...");

    /* 当前状态初始化 */
    /* 缓存消息队列初始化 */
    pstMMCFsmMsgBuf                       = NAS_LMM_GetFsmBufQueueAddr(NAS_LMM_PARALLEL_FSM_MMC);
    pstMMCFsmMsgBuf->ucHighPrioCnt        = NAS_EMM_MSG_BUF_EMPTY;
    pstMMCFsmMsgBuf->ucMidPrioCnt         = NAS_EMM_MSG_BUF_EMPTY;
    pstMMCFsmMsgBuf->ucLowPrioCnt         = NAS_EMM_MSG_BUF_EMPTY;
    for ( ulCont = 0; ulCont < NAS_EMM_BUF_MSG_MAX_NUM; ulCont++ )
    {
        pstMMCFsmMsgBuf->astHighPrioBufQue[ulCont].pBuffMsg = NAS_LMM_NULL_PTR;
        pstMMCFsmMsgBuf->astMidPrioBufQue[ulCont].pBuffMsg = NAS_LMM_NULL_PTR;
        pstMMCFsmMsgBuf->astLowPrioBufQue[ulCont].pBuffMsg = NAS_LMM_NULL_PTR;
    }
    #endif
}
