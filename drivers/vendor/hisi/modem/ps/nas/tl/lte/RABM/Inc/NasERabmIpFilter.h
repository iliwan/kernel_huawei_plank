...
#if 0
/* IPF过滤结果错误检查 */
#define NAS_ERABM_IPF_RESULT_ERR_CHECK(pstIpfResult)\
       ((NAS_ERABM_OP_TRUE == pstIpfResult->usVerErr)||\
       (NAS_ERABM_OP_TRUE == pstIpfResult->usBdCdNotEq)||\
       (NAS_ERABM_OP_TRUE == pstIpfResult->usPktLenNotEq)||\
       (NAS_ERABM_OP_TRUE == pstIpfResult->usPktLenErr)||\
       (NAS_ERABM_OP_TRUE == pstIpfResult->usHdrLenErr))
#endif
...
#if 0
/* IPF下行Filter定义 */
#define NAS_ERABM_IPF_DL_FILTER_DEFINE_BEGIN(filter_id)\
    pstFilter = &g_astNasERabmDlUserFilter[filter_id];\
    NAS_ERABM_MEM_SET(pstFilter, 0x0, sizeof(NAS_ERABM_IPF_DL_FILTER_INFO))

#define NAS_ERABM_IPF_DL_FILTER_IP_TYPE(iptype)\
    pstFilter->ucIpType = (iptype)

#define NAS_ERABM_IPF_DL_FILTER_PROTOCOL(protocol)\
    pstFilter->ucProtocolId = (protocol);\
    pstFilter->bitOpProtocolId = NAS_ERABM_OP_TRUE

#define NAS_ERABM_IPF_DL_FILTER_PORT_HI(port_hi)\
    pstFilter->usPortHi = (port_hi);\
    pstFilter->bitOpPort = NAS_ERABM_OP_TRUE

#define NAS_ERABM_IPF_DL_FILTER_PORT_LO(port_lo)\
    pstFilter->usPortLo = (port_lo);\
    pstFilter->bitOpPort = NAS_ERABM_OP_TRUE

#define NAS_ERABM_IPF_DL_FILTER_ICMPTYPE(type)\
    pstFilter->usIcmpType = (type);\
    pstFilter->bitOpType = NAS_ERABM_OP_TRUE

#define NAS_ERABM_IPF_DL_FILTER_ICMPCODE(code)\
    pstFilter->usIcmpCode = (code);\
    pstFilter->bitOpCode = NAS_ERABM_OP_TRUE

#define NAS_ERABM_IPF_DL_FILTER_DEFINE_END(filter_id)

/* 下行DHCPv4 Filter定义, UDP 目的端口为68 */
#define NAS_ERABM_IPF_DEF_DHCPV4_FILTER()\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_BEGIN(NAS_ERABM_IPF_DL_FITLER_DHCP4);\
    NAS_ERABM_IPF_DL_FILTER_IP_TYPE(0);\
    NAS_ERABM_IPF_DL_FILTER_PROTOCOL(17);\
    NAS_ERABM_IPF_DL_FILTER_PORT_HI(68);\
    NAS_ERABM_IPF_DL_FILTER_PORT_LO(68);\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_END(NAS_ERABM_IPF_DL_FITLER_DHCP4)

/* 下行DHCPv6 Filter定义, UDP目的端口546 */
#define NAS_ERABM_IPF_DEF_DHCPV6_FILTER()\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_BEGIN(NAS_ERABM_IPF_DL_FITLER_DHCP6);\
    NAS_ERABM_IPF_DL_FILTER_IP_TYPE(1);\
    NAS_ERABM_IPF_DL_FILTER_PROTOCOL(17);\
    NAS_ERABM_IPF_DL_FILTER_PORT_HI(546);\
    NAS_ERABM_IPF_DL_FILTER_PORT_LO(546);\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_END(NAS_ERABM_IPF_DL_FITLER_DHCP6)

/*下行ICMP Router Solicitation Filter定义, 协议号58，Type 133， code 0 */
#define NAS_ERABM_IPF_DEF_ICMP_RS_FILTER()\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_BEGIN(NAS_ERABM_IPF_DL_FITLER_RS);\
    NAS_ERABM_IPF_DL_FILTER_IP_TYPE(1);\
    NAS_ERABM_IPF_DL_FILTER_PROTOCOL(58);\
    NAS_ERABM_IPF_DL_FILTER_ICMPTYPE(133);\
    NAS_ERABM_IPF_DL_FILTER_ICMPCODE(0);\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_END(NAS_ERABM_IPF_DL_FITLER_RS)

/*下行ICMP Router Advertisement Filter定义, 协议号58，Type 134， code 0 */
#define NAS_ERABM_IPF_DEF_ICMP_RA_FILTER()\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_BEGIN(NAS_ERABM_IPF_DL_FITLER_RA);\
    NAS_ERABM_IPF_DL_FILTER_IP_TYPE(1);\
    NAS_ERABM_IPF_DL_FILTER_PROTOCOL(58);\
    NAS_ERABM_IPF_DL_FILTER_ICMPTYPE(134);\
    NAS_ERABM_IPF_DL_FILTER_ICMPCODE(0);\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_END(NAS_ERABM_IPF_DL_FITLER_RA)

/*下行ICMP Neighbor Solicitation Filter定义, 协议号58，Type 135， code 0 */
#define NAS_ERABM_IPF_DEF_ICMP_NS_FILTER()\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_BEGIN(NAS_ERABM_IPF_DL_FITLER_NS);\
    NAS_ERABM_IPF_DL_FILTER_IP_TYPE(1);\
    NAS_ERABM_IPF_DL_FILTER_PROTOCOL(58);\
    NAS_ERABM_IPF_DL_FILTER_ICMPTYPE(135);\
    NAS_ERABM_IPF_DL_FILTER_ICMPCODE(0);\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_END(NAS_ERABM_IPF_DL_FITLER_NS)

/*下行ICMP Neighbor Advertisement Filter定义, 协议号58，Type 136， code 0 */
#define NAS_ERABM_IPF_DEF_ICMP_NA_FILTER()\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_BEGIN(NAS_ERABM_IPF_DL_FITLER_NA);\
    NAS_ERABM_IPF_DL_FILTER_IP_TYPE(1);\
    NAS_ERABM_IPF_DL_FILTER_PROTOCOL(58);\
    NAS_ERABM_IPF_DL_FILTER_ICMPTYPE(136);\
    NAS_ERABM_IPF_DL_FILTER_ICMPCODE(0);\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_END(NAS_ERABM_IPF_DL_FITLER_NA)

/*下行ICMP REDIRECT Filter定义, 协议号58，Type 137， code 0 */
#define NAS_ERABM_IPF_DEF_ICMP_REDIRECT_FILTER()\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_BEGIN(NAS_ERABM_IPF_DL_FITLER_REDIRECT);\
    NAS_ERABM_IPF_DL_FILTER_IP_TYPE(1);\
    NAS_ERABM_IPF_DL_FILTER_PROTOCOL(58);\
    NAS_ERABM_IPF_DL_FILTER_ICMPTYPE(137);\
    NAS_ERABM_IPF_DL_FILTER_ICMPCODE(0);\
    NAS_ERABM_IPF_DL_FILTER_DEFINE_END(NAS_ERABM_IPF_DL_FITLER_REDIRECT)

/* 获取下行用户定义的Filter */
#define NAS_ERABM_IPF_GET_DL_USER_FILTER(filter_id)\
    (&g_astNasERabmDlUserFilter[filter_id])

/* 查表找到Filter ID对应的应配置给驱动的Bid */
#define NAS_ERABM_IPF_CONV_DLFILTER_TO_BID(filter_id)\
    (g_aulNasERabmIpfFilterTypeTable[filter_id][1])

/* 查表找到Filter ID 对应的Filter类型 */
#define NAS_ERABM_IPF_CONV_DLFILTER_TO_TYPE(filter_id)\
    (g_aulNasERabmIpfFilterTypeTable[filter_id][2])
#endif
/* IPF上行NDIS Filter定义 */
...
#if 0
/*****************************************************************************
 结构名    : NAS_ERABM_IPF_DL_FILTER_INFO
 协议表格  :
 ASN.1描述 :
 结构说明  : 下行方向Filter使用的简易结构，用于IPM 与RABM间交互
*****************************************************************************/
typedef struct
{
...
*****************************************************************************/
#if 0
extern VOS_UINT32  NAS_ERABM_IpfCommonConfig(VOS_VOID);
#endif
...
extern VOS_UINT32 NAS_ERABM_IpfConfigDlFilter(VOS_VOID);

extern VOS_UINT32 NAS_ERABM_IpfAddDlFilter(const IPF_FILTER_CONFIG_S *pstFilter);

extern VOS_UINT32 NAS_ERABM_IpfRemoveDlFilter(VOS_UINT32 ulFilterId);

extern VOS_VOID NAS_ERABM_IpfSendDataToIpm(const TTF_MEM_ST *pstData, VOS_UINT8 ucEpsbId);

extern VOS_UINT32 NAS_ERABM_IpfAddUserFilter(VOS_UINT32 ulFilterType);

extern VOS_UINT32 NAS_ERABM_IpfRmvUserFilter(VOS_UINT32 ulFilterId);

extern VOS_VOID NAS_ERABM_IpfDefDlFilter(VOS_VOID);

extern VOS_UINT32 NAS_ERABM_IpfExtractEpsbId
(
    const LUP_MEM_ST *pstSdu,
    VOS_UINT32 *pulEpsbId
);
extern PS_BOOL_ENUM_UINT8 LNAS_ERABM_IpfDlRdProc(TTF_MEM_ST *pstTtfMem);
#endif
