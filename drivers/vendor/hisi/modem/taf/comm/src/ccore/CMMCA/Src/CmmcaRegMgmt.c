...
)
{
    ...
    pstRegCtx->ucSrvDomain = TAF_PH_SERVICE_PS;
    ...
    {
        stSysCfgPara.ucRoam = TAF_PH_ROAM_UNCHANGE;
    }
...
)
{
    ...
    stSysInfo.stEutran.usEarfcn = pstRegCtx->usArfcn;
...
)
{
    ...
    stRegPara.ucAttachDomain       = TAF_PH_SERVICE_PS;
...
)
{
    ...
    stRegPara.ucAttachDomain       = TAF_PH_SERVICE_PS;
    stRegPara.stCellInfo.usArfcn   = pstCellInfoPsRegReq->stSystemChangeReq.stTargetSystemCarrierInfo.stCarrierInfoEutran.usEarfcn;
...
)
{
    ...
    pstRegCtx->usArfcn      = pstAcqCnf->usArfcn;
    ...
    stRatInfo.usEarfcn        = pstAcqCnf->usArfcn;
...
)
{
    ...
    else
    {
        ...
        stRatInfo.usEarfcn        = pstRegCtx->usArfcn;
    }
...
)
{
    ...
    pstRegCtx->usArfcn          = pstAcqInd->usArfcn;
    ...
    stRatInfo.usEarfcn        = pstAcqInd->usArfcn;
...
)
{
 ...
 )
 {
    ...
    pstRegCtx->usArfcn          = pstRegCnf->usArfcn;
    ...
    stRatInfo.usEarfcn        = pstRegCnf->usArfcn;
    CMMCA_REG_SndRegRsp(pstRegCnf->enRslt, usCmdId, &stRatInfo);
...
)
{
    ...
    pstRegCtx->usArfcn      = pstSysInfoInd->usArfcn;
    ...
    stRatInfo.usEarfcn        = pstSysInfoInd->usArfcn;
...
)
{
    ...
    if (TAF_MMA_APP_OPER_RESULT_SUCCESS == enRslt)
    {
        if (ID_CMMCA_MMC_RAT_PS_REG_REQ == usCmdId)
        {
            CMMCA_REG_SndPsRegRsp(CMMCA_MMC_RAT_ID_EUTRAN, CMMCA_RAT_REG_RESULT_REGISTERED, pstRatInfo);
        }
        else
        {
            CMMCA_REG_SndCellInfoPsRegRsp(CMMCA_MMC_RAT_ID_EUTRAN, CMMCA_RAT_MMC_STATUS_SUCCESS, pstRatInfo);
        }
    ...
    else if (TAF_MMA_APP_OPER_RESULT_FAILURE == enRslt)
    {
        if (ID_CMMCA_MMC_RAT_PS_REG_REQ == usCmdId)
        {
            CMMCA_REG_SndPsRegRsp(CMMCA_MMC_RAT_ID_EUTRAN, CMMCA_RAT_REG_RESULT_ACQUIRED, pstRatInfo);
        }
        else
        {
            CMMCA_REG_SndCellInfoPsRegRsp(CMMCA_MMC_RAT_ID_EUTRAN, CMMCA_RAT_MMC_STATUS_PS_REG_FAIL, pstRatInfo);
        }
    }
    /* 搜网失败 */
    else if (TAF_MMA_APP_OPER_RESULT_ACQUIRED_FAIL == enRslt)
    {
        if (ID_CMMCA_MMC_RAT_PS_REG_REQ == usCmdId)
        {
            CMMCA_REG_SndPsRegRsp(CMMCA_MMC_RAT_ID_EUTRAN, CMMCA_RAT_REG_RESULT_ACQFAILED, pstRatInfo);
        }
        else
        {
            CMMCA_REG_SndCellInfoPsRegRsp(CMMCA_MMC_RAT_ID_EUTRAN, CMMCA_RAT_MMC_STATUS_ACQ_FAIL, pstRatInfo);
        }
    }
    else
    {
