...
/* TDD模式上下行最低频率, TDD:E-UTRA Operatingn Band 取33~40 */
...
/* NOffs-DL or NOffs-UL, TDD:E-UTRA Operatingn Band 取33~43 */
...
)
{
    ...
    usBandInfo  = MTC_GAS_BAND_TYPE_UNKNOWN;
...
VOS_UINT16  MTC_LTE_GetBandInfoByDlArfcn( VOS_UINT16 usDlEarfcn)
{
    VOS_UINT8                           ucLoop = 0;
    VOS_UINT16                          usFreqBand;
    VOS_UINT16                          ausDlEarfcn[][2] = {{0, 0},
      ...
      {39650, 41589}, {41590, 43589}, {43590, 45589}};                  /* Band41~Band43 */

    /* 初始化频带指示 */
    usFreqBand = 0;

    /* 根据36.101协议中表 5.7.3-1推算DlEarfcn所在的频带 */
    for (ucLoop = MTC_FREQ_BAND1; ucLoop <= MTC_FREQ_BAND43; ucLoop++)
    {
        if ((usDlEarfcn >= ausDlEarfcn[ucLoop][0]) && (usDlEarfcn <= ausDlEarfcn[ucLoop][1]))
        {
            usFreqBand = ucLoop;
...
)
{
    ...
    /* 频带指示1~14,17~21,24为FDD频带，33~43为TDD频带 */
    if (MTC_COMM_CHECK_IS_FDD_BAND_IND(ucFreqBandIndicator))
    {
        ...
        usDlFreq = (g_ausFreqDlLowFdd[ucFreqBandIndicator-1] + usDlEarfcn - g_ausNOffsetDlFdd[ucFreqBandIndicator-1]);
    }
    else if (MTC_COMM_CHECK_IS_PROCOL_TDD_BAND_IND(ucFreqBandIndicator))
    {
        /* 36.101: FDL = FDL_low + 0.1(NDL - NOffs-DL), 下行频率FDL的单位为MHZ，
        为了去浮点化，对公式放大10倍处理，所以实际计算频率单位为100KHZ */
        usDlFreq = (g_ausFreqLowTdd[ucFreqBandIndicator-33] + usDlEarfcn - g_ausNOffsetTdd[ucFreqBandIndicator-33]);
    }
    else
    {
    }
...
)
{
    ...
    {
        usArfNum         = pstUsingFreq->stLteFreqInfo.ausLteArfcnList[ulIndex];
        usBandNum        = MTC_LTE_GetBandInfoByDlArfcn(usArfNum);
        pstRfFreqList->stLteFreq.ausFreq[ulIndex] = MTC_CalcLteDlFreq(usBandNum, usArfNum);

        /* 记录可维可测信息 */
        MTC_DEBUG_SAVE_AS_RF_USING_INFO_NUM(stLteFreq);
        MTC_DEBUG_SAVE_RF_USING_INFO(stLteFreq, usArfcn, usArfNum, ulIndex);
