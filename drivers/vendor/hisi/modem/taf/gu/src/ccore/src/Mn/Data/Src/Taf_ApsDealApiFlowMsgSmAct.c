...
)
{
    ...
    /*打包向SM发送的MSG;*/
    if ( APS_FAIL == Aps_PackSmActReqMsg (pActReq, pstPdpActReq))
    {
...
)
{
    ...
    pStPdpActReq->bitOpAcsPointName = APS_FREE;
    pStPdpActReq->bitOpProtCfgOpt   = APS_FREE;
    pStPdpActReq->bitOpSapi         = APS_FREE;
    pStPdpActReq->bitOpSpare        = APS_FREE;

    /* 填写 ConnectId (CR)，当前CR等同于Pdp Id */
    pStPdpActReq->ucConnectId       = pActReq->ucPdpId;

    /*LLC SAPI*/
    pStPdpActReq->bitOpSapi         = APS_USED;
    ...
    {
        /*如果APN长度为0，则不选APN*/
        pStPdpActReq->bitOpAcsPointName = APS_NOT_SELECT_THIS_FIELD;
    }
    else
    {
        pStPdpActReq->bitOpAcsPointName = APS_SELECT_THIS_FIELD;
    ...
    {
        /*PCO*/
        pStPdpActReq->bitOpProtCfgOpt   = APS_USED;
        if ( APS_FAIL == Aps_PackSmPco(pActReq, &pStPdpActReq->stProtCfgOpt))
        {
...
)
{
    ...
    /*发起激活的方式*/
    g_PdpEntity[pActReq->ucPdpId].PdpProcTrackFlag  = APS_USED;
    ...
    /*QOS */
    g_PdpEntity[ucPdpId].PdpQosFlag                 = APS_USED;
    ...
    if (pActReq->OP_AUTH)
    {
        ...
        {
            g_PdpEntity[ucPdpId].PdpAuthFlag        = APS_USED;
    ...
    /*Dns*/
    if (  (APS_USED == pActReq->OP_DNS)
        &&(  (APS_USED == pActReq->PdpDns.bitOpPrimDnsAddr)
           ||(APS_USED == pActReq->PdpDns.bitOpSecDnsAddr) ))
    {
        /*有一个有效的DNS，就填状态机*/
        g_PdpEntity[ucPdpId].PdpDnsFlag             = APS_USED;

        if (APS_USED == pActReq->PdpDns.bitOpPrimDnsAddr)
        {
            g_PdpEntity[ucPdpId].PdpDns.OP_PriDns   = APS_USED;
        ...
        if (APS_USED == pActReq->PdpDns.bitOpSecDnsAddr)
        {
            g_PdpEntity[ucPdpId].PdpDns.OP_SecDns   = APS_USED;
    ...
    /*TFT*/
    g_PdpEntity[ucPdpId].PdpTftFlag                 = APS_FREE;
...
VOS_UINT32  Aps_PdpSecActReqParaCheck ( APS_PDP_SEC_ACT_REQ_ST *pSecActReq)
{
    ...
    {
        if( APS_ERROR == Aps_TftValidOrNot(&pSecActReq->Tft))
        {
            return  APS_PARA_INVALID;
        }
    }
...
)
{
    ...
    pStSecActReq->bitOpSapi         = APS_FREE;
    pStSecActReq->bitOpTft          = APS_FREE;
    pStSecActReq->bitOpSpare        = APS_FREE;

    /*LLC SAPI*/
    pStSecActReq->bitOpSapi         = APS_USED;
    ...
    if ( APS_USED == pSecActReq->Op_Tft )
    {
        pStSecActReq->bitOpTft      = APS_USED;
                                            ...
                                            SMREG_PDP_MODIFY_REQ_STRU       *pStPdpMdfReq   )
{
    ...
    pStPdpMdfReq->bitOpRequestedNewQos  = APS_FREE;
    pStPdpMdfReq->bitOpNsapi            = APS_FREE;
    pStPdpMdfReq->bitOpNewtft           = APS_FREE;
    pStPdpMdfReq->bitOpSapi             = APS_FREE;
    pStPdpMdfReq->bitOpSpare            = APS_FREE;

    /*SAPI*/
    pStPdpMdfReq->bitOpSapi             = APS_USED;
    pStPdpMdfReq->ucSapi                = pMdfReq->ucSapi;

    /*Qos*/
    if ( pMdfReq->Op_Qos)
    {
        pStPdpMdfReq->bitOpRequestedNewQos = APS_SELECT_THIS_FIELD;
    ...
    if ( pMdfReq->Op_Tft)
    {
        pStPdpMdfReq->OP_Newtft         = APS_SELECT_THIS_FIELD;
    ...
    /*向MODIRY_REQ中填写NSAPI */
    pStPdpMdfReq->bitOpNsapi            = APS_SELECT_THIS_FIELD;
...
)
{
    ...
    if ( ( APS_FREE == g_PdpEntity[ucPdpId].PdpAddrFlag)
                                        ...
                                        VOS_UINT8               ucRelSmType)
{
    ...
    /*消息初始化*/
    pStDeActReq->bitOpTearDownInd       = APS_FREE;
    pStDeActReq->bitOpNsapi             = APS_FREE;
    pStDeActReq->bitOpPcpTaCause        = APS_FREE;
    pStDeActReq->bitOpSpare             = APS_FREE;

    /*填充消息*/
    pStDeActReq->bitOpTearDownInd       = APS_USED;
    pStDeActReq->ucTearDownInd          = pDeActReq->ucTearDown;

    /*不填NSAPI，因为清资源时可能还没有分配NSAPI
    pStDeActReq->OP_Nsapi               = APS_USED;
    ...
    */
    ...
    pStDeActReq->bitOpPcpTaCause        = APS_USED;
...
/*****************************************************************************
 Prototype      : Aps_CompAddr()
 Description    : 比较ucReqPdpId和ucDstPdpId所指向的PDP实体的IP地址是否相同
 Input          : ucReqPdpId,
                  ucDstPdpId
 Output         : NO
 Return Value   : #define APS_ADDR_SAME                   0
...
{
    if (APS_FREE == g_PdpEntity[ucDstPdpId].PdpAddrFlag)
    {
        return  APS_ADDR_DIFF;
    }

    /*PDP类型*/
    if (g_PdpEntity[ucReqPdpId].PdpAddr.ucPdpTypeNum
        != g_PdpEntity[ucDstPdpId].PdpAddr.ucPdpTypeNum )
    {
        return  APS_ADDR_DIFF;
    }

    /*4个字节的地址*/
    if ( 0 != VOS_MemCmp(   g_PdpEntity[ucReqPdpId].PdpAddr.aucIpAddr,
                            g_PdpEntity[ucDstPdpId].PdpAddr.aucIpAddr,
                            TAF_IPV4_ADDR_LEN))
    {   /*地址不同*/
        return  APS_ADDR_DIFF;
    }

    return  APS_ADDR_SAME;
}


/*****************************************************************************
 Prototype      : Aps_CompApn()
 Description    : 比较ucReqPdpId和ucDstPdpId所指向的PDP实体的APN是否相同
 Input          : ucReqPdpId,
                  ucDstPdpId
 Output         : NO
 Return Value   : #define APS_ADDR_SAME                   0
...
VOS_UINT8   Aps_CompApn(        VOS_UINT8           ucReqPdpId,
                                        VOS_UINT8           ucDstPdpId  )
{
    if (APS_FREE == g_PdpEntity[ucReqPdpId].PdpApnFlag)
    {
        return  APS_APN_DIFF;
    }

    if (APS_FREE == g_PdpEntity[ucDstPdpId].PdpApnFlag)
    {
        return  APS_APN_DIFF;
    }

    if (g_PdpEntity[ucReqPdpId].PdpApn.ucLength
         != g_PdpEntity[ucDstPdpId].PdpApn.ucLength)
    {   /*APN不同*/
        return  APS_APN_DIFF;
    }

    if ( 0 != VOS_MemCmp(   g_PdpEntity[ucReqPdpId].PdpApn.aucValue,
                            g_PdpEntity[ucDstPdpId].PdpApn.aucValue,
                            g_PdpEntity[ucReqPdpId].PdpApn.ucLength))
    {   /*APN不同*/
        return  APS_APN_DIFF;
    }
...
/*******************************************************************************
Function:       Aps_TftValidOrNot()
Description:    检查TFT Parameter 是否合法, 只检查FilterId。
Calls:
Called :
Date Accessed :
Date Update  :
Input:          指针, 指向APS模块定义的TFT STRUCT 。
Output:
Return:         APS_SUCCESS,  Parameter 合法
                APS_ERROR, 函数执行 error , 或 Parameter  not valid
Others:
*******************************************************************************/
VOS_UINT32 Aps_TftValidOrNot( APS_PDP_TFT_ST   *psTft)
{
    if( VOS_NULL == psTft)
    {
        APS_WARN_LOG("The input pointer parameter can not be NULL ");
        return(APS_ERROR);
    }

    /* ulPacketFilterId 不属于闭区间[0,7]则 not valid  */
    if((psTft->ucPacketFilterId > APS_TFT_FILTERID_TOP))
    {
        APS_WARN_LOG("ERROR! PacketFilterId not valid ! Aps_TftValidOrNot()");
        return(APS_ERROR);
    }
    else
    {
...
)
{
    ...
    if ( (APS_QOS_TRAFFIC_CLASS_INTERACTIVE == psQos->ucTrafficClass) )
    {
        if (APS_SUCC != MN_APS_QosValidOrNot_ForTrafficHandlingPriority(psQos))
        {
            return (APS_ERROR);
        }
    }
    ...
    #if(PS_UE_REL_VER >= PS_PTL_VER_R5)
    if (APS_SUCC != Aps_QosValidOrNot_ForSrcStatisticsDescriptor(psQos))
    {
        return(APS_ERROR);
    }

    if (APS_SUCC != Aps_QosValidOrNot_ForSignallingIndication(psQos))
    {
        return(APS_ERROR);
    }

    if (APS_SUCC != Aps_QosValidOrNot_ForMaxBitRateForDownExt(psQos))
    {
        return(APS_ERROR);
    }
    ...
    {
        if (APS_SUCC != Aps_QosValidOrNot_ForGuarantBitRateForDownExt(psQos))
        {
            return APS_ERROR;
        }
    }
    ...
    #if(PS_UE_REL_VER >= PS_PTL_VER_R7)
    if (APS_SUCC != Aps_QosValidOrNot_ForMaxBitRateForUpExt(psQos))
    {
        return(APS_ERROR);
    }
    ...
    {
        if (APS_SUCC != Aps_QosValidOrNot_ForGuarantBitRateForUpExt(psQos))
        {
            return APS_ERROR;
        }
    }
...
/*****************************************************************************
 Prototype      : Aps_FindPdpContext()
 Description    : 首先判断PdpId的合法性, 并根据合法的PdpId找到PDP实体, 并返回指向
                  该实体的指针。若PdpId非法, 则返回空指针。
 Input          : ucPdpId,[0,6]和[8,14],待找的PDP的PdpId
 Output         : NO
 Return Value   : struct APS_PDP_CONTEXT_ENTITY_STRU *p
                  指针型,指向The Input ucPdpId对应的PDP上下文的指针。
                  若error,返回空指针。
 Date Accessed  : g_ApsPdpContextTable
 Date Update    :
 Calls          :
 Called By      :
 History        : ---
...
{
    if (   (ucPdpId > APS_MAX_PDP_NUM)
        || (ucPdpId == APS_FREE_ENTITY_NUM)) /*PdpId合法性判断 */
    {
        APS_LOG1("PdpId  ERROR ! PdpId = ", ucPdpId);
        return(VOS_NULL);
    }

    return( &(g_PdpEntity[ucPdpId]) );
}
