...
)
{
    ...
    if (ucOldSrvSta != TAF_SDC_GetServiceStatus())
    {
...
)
{
    ...
    TAF_MMA_ReportRegStatus(pstServiceStatus->enRegState, pstServiceStatus->enCnDomainId);
...
)
{
    ...
    /* MMC通知MMA注册状态时，是否需要上报给AT */
    if (VOS_TRUE == TAF_MMA_IsNeedReportRegStatus_RegStatusInd(enCnDomainId, enOldCsRegState, enOldPsRegState, enRegState))
    {
        if (MMA_MMC_SRVDOMAIN_CS == enCnDomainId)
        {
            /* 上报CREG，更新CS域注册状态 */
            TAF_MMA_SndAtRegStatus(MMA_MMC_SRVDOMAIN_CS, enRegState);
            TAF_SDC_SetCsRegStatus(enRegState);
        }
        else if (MMA_MMC_SRVDOMAIN_PS == enCnDomainId)
        {
            /* 上报CGREG/CEREG，更新PS域注册状态 */
            TAF_MMA_SndAtRegStatus(MMA_MMC_SRVDOMAIN_PS, enRegState);
...
)
{
    ...
    /* rat是否变化 */
    if ((pstOldPlmnInfo->enServiceSysMode) != (pstAppNwInfo->stServPlmnInfo.enServiceSysMode))
    {
        return VOS_TRUE;
    }

    /* PLMN是否变化 */
    if ( (pstOldPlmnInfo->stServicePlmnId.ulMcc) != (pstAppNwInfo->stServPlmnInfo.stServicePlmnId.ulMcc)
      || (pstOldPlmnInfo->stServicePlmnId.ulMnc) != (pstAppNwInfo->stServPlmnInfo.stServicePlmnId.ulMnc) )
    {
        return VOS_TRUE;
    }

    /* lac是否变化 */
    if ((pstOldPlmnInfo->usServiceLac) != (pstAppNwInfo->stServPlmnInfo.usServiceLac))
    {
        return VOS_TRUE;
    }

    /* rac是否变化 */
    if ((pstOldPlmnInfo->ucServiceRac) != (pstAppNwInfo->stServPlmnInfo.ucServiceRac))
    {
        return VOS_TRUE;
    }
    
    /* cellid是否变化 */
    if ((pstOldPlmnInfo->ulServiceCellId) != (pstAppNwInfo->stServPlmnInfo.ulServiceCellId))
    {
        return VOS_TRUE;
    }
