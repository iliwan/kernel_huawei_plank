...
VOS_UINT32 TAF_SSA_ProcUssdMode(
...
VOS_VOID  Taf_SsMsgReq(ST_SSP_MSG *pMsg)
{
     ...
     && (D_SMC_BEGIN_REQ == pMsg->SspmsgCore.ucChoice))
    {
        ...
        TAF_SSA_StartTimer(TI_TAF_SSA_RETRY_PERIOD_TIMER,
...
VOS_UINT32   SSA_MmiGetPwd(SSA_MMI_PARA_STRU *pstPara,  VOS_UINT8  *pMMIStr, VOS_UINT8  ucStrLen)
{
    ...
    /*拷贝旧密码*/
    VOS_MemCpy(pstPara->aucOldPwd, pucTmp, 4);
    pucTmp = pucTmp + 4;
    ...
    /*拷贝新密码*/
    VOS_MemCpy(pstPara->aucNewPwd, pucTmp, 4);
    pucTmp = pucTmp + 4;
    ...
    /*拷贝确认密码*/
    VOS_MemCpy(pstPara->aucNewPwdCnf, pucTmp, 4);
    pucTmp = pucTmp + 4;
...
VOS_UINT32   SSA_GetMmiPara(SSA_MMI_PARA_STRU *pstPara,  VOS_UINT8  *pMMIStr, VOS_UINT8  ucStrLen)
{
    ...
    /*lint +e961*/    
    {
        ...
        if (('*' == *pucTmp) || ('#' == *pucTmp))
        {
            ...
            else if ((!pstPara->ucSiaFlg) && ('*' == *pucTmp))
            {
                pstPara->ucSiaFlg = 1;
                pucTmp++;
                continue;
            ...
            else if ((!pstPara->ucSibFlg) && ('*' == *pucTmp))
            {
            ...
            else if ((!pstPara->ucSicFlg) && ('*' == *pucTmp))
            {
