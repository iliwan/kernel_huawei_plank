...
VOS_VOID SSA_EncodeErrorCode(VOS_UINT8 *pucContent, VOS_UINT8 *pucCodeLen, VOS_UINT16 usErrCode)
{
    ...
    /*内存移位拷贝*/
    memmove(pucContent + 3, pucContent, *pucCodeLen);
    memcpy(pucContent, ucTmp, 3);
    *pucCodeLen += 3;
}
...
VOS_VOID SSA_EncodeSequenceTag(VOS_UINT8 *pucContent, VOS_UINT8 *pucCodeLen)
{
    ...
    /*内存移位拷贝*/
    memmove(pucContent + 2, pucContent, *pucCodeLen);
    memcpy(pucContent, ucTmp, 2);
    *pucCodeLen += 2;
}
...
VOS_VOID SSA_EncodeComponentType(VOS_UINT8 *pucContent, VOS_UINT8 *pucCodeLen, VOS_UINT8 ucComponentTag)
{
    ...
    /*内存移位拷贝*/
    memmove(pucContent + ucheaderLen, pucContent, *pucCodeLen);
    memcpy(pucContent, ucTmp, ucheaderLen);
                                ...
                                VOS_UINT8 *pucBcdNumLen, VOS_UINT8 *pucNewNumType)
{
    ...
    if ('+' == *(pucAsciiNum))
    {
        ...
        PS_MEM_MOVE(pucAsciiNum, pucAsciiNum + 1, ucNewAsciiNumLen - 1);
        ucNewAsciiNumLen--;
...
VOS_UINT32  SSA_PackTL(VOS_UINT8  ucTag,  VOS_UINT8   *pucContent,  VOS_UINT8 *pucCodeLen)
{
    ...
    VOS_MemMove(pucContent + ucheaderLen, pucContent, *pucCodeLen);
...
/*****************************************************************************
 ...
 History        : ---
  1.Date        : 2005-08-15
    ...
    Modification: Created function
*****************************************************************************/
VOS_UINT32  SSA_EncodeLength(VOS_UINT8 ucSrcLen, VOS_UINT8 *pucContent, VOS_UINT8 *pucCodeLen)
{
