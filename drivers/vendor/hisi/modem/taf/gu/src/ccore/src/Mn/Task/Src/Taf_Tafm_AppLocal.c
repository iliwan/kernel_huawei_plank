...
VOS_VOID Api_AppRabmDataInd(ST_APP_RABM_DATA *p)
{
    ...
    if (VOS_NULL_PTR == p)
    {
        PS_LOG( WUEPS_PID_RABM, 0, PS_PRINT_WARNING, "Api_AppRabmDataInd:WARNING:p is null ptr." );
        return;
    }
    if ((p->ucDomain > TAF_DATA_PS_DOMAIN) ||
        (p->ucRabId > TAF_MAX_RABID))
    {
        PS_LOG( WUEPS_PID_RABM, 0, PS_PRINT_WARNING, "Api_AppRabmDataInd:WARNING:Domain or RabId value is error." );
        return;
    }
    ...
    {
        PS_LOG( WUEPS_PID_RABM, 0, PS_PRINT_WARNING, "Api_AppRabmDataInd:WARNING:Can't find match ClientId." );
        return;/*未找到ClientId*/
    }

    if ( VOS_NULL_PTR == g_stMnCallBackFunc[i].regFunc.pDataIndFunc )
    {
        PS_LOG1( WUEPS_PID_RABM, 0, PS_PRINT_WARNING, "Api_AppRabmDataInd:WARNING:Function Pointer is NULL! g_stMnCallBackFunc array index:", i );
        return;
    }
...
VOS_VOID Api_AppRabmStatusInd(ST_APP_RABM_STATUS *p)
{
    ...
    if (VOS_NULL_PTR == p)
    {
        PS_LOG( WUEPS_PID_RABM, 0, PS_PRINT_WARNING, "Api_AppRabmStatusInd:WARNING:p is null ptr." );
        return;
    }
    if ((p->ucDomain > TAF_DATA_PS_DOMAIN) ||
        (p->ucRabId > TAF_MAX_RABID))
    {
        PS_LOG( WUEPS_PID_RABM, 0, PS_PRINT_WARNING, "Api_AppRabmStatusInd:WARNING:Domain or RabId value is error." );
        return;
    }
...
VOS_VOID Api_AppRabmErrInd(ST_APP_RABM_ERROR *p)
{
    ...
    if (VOS_NULL_PTR == p)
    {
        PS_LOG( WUEPS_PID_RABM, 0, PS_PRINT_WARNING, "Api_AppRabmErrInd:WARNING:p is null ptr." );
        return;
    }
    if ((p->ucDomain > TAF_DATA_PS_DOMAIN) ||
        (p->ucRabId > TAF_MAX_RABID))
    {
        PS_LOG( WUEPS_PID_RABM, 0, PS_PRINT_WARNING, "Api_AppRabmErrInd:WARNING:Domain or RabId value is error." );
        return;
    }
